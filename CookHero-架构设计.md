...
对应技术选型：[CookHero-技术选型.md](./CookHero-技术选型.md)
...
flowchart TB
  U[User] --> W[Web App]
  W --> G[API Gateway / BFF]
  G --> O[Agent Orchestrator / Spring Boot]
...
- 聚合返回
- 流式协议转发
- Java 实现时可用 Spring Cloud Gateway 或独立 Spring Boot BFF 承担这一层
...
- 必须记录每一步
- 必须支持失败回退
- Java 实现时建议将编排器拆成领域服务、状态机和执行上下文对象
...
- 计划不合理
- 回答和工具结果不一致
- Java 侧建议将 Validator 做成独立领域服务，避免散落在 Controller 或 Orchestrator 中
...
- 参数错误不可重试
- 每个步骤有最大重试次数
- Java 可结合 Resilience4j 实现重试、熔断和限流
...
flowchart LR
  ...
  Web --> API[API Service / Spring Boot]
  API --> Agent[Agent Service / Spring Boot]
  Agent --> DB[(PostgreSQL)]
  Agent --> Redis[(Redis)]
  Agent --> MinIO[(MinIO)]
  Agent --> Worker[Worker]
...
flowchart TB
  ...
  CDN --> API[API Gateway / Spring Boot]
  API --> Agent[Agent Orchestrator / Spring Boot]
