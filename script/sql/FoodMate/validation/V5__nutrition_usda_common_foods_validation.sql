-- V5 common nutrition directory seed validation. Read-only.

SELECT nutrition_food_id,
       standard_name,
       chinese_name,
       basis_unit,
       calories_kcal_per_100,
       protein_g_per_100,
       fat_g_per_100,
       carbs_g_per_100,
       source_name,
       source_version,
       review_status
FROM nutrition_foods
WHERE nutrition_food_id BETWEEN 510017 AND 510025
ORDER BY nutrition_food_id;

SELECT COUNT(*) AS common_nutrition_seed_rows
FROM nutrition_foods
WHERE nutrition_food_id BETWEEN 510017 AND 510025
  AND is_deleted = FALSE;

SELECT COUNT(*) AS invalid_common_nutrition_seed_rows
FROM nutrition_foods
WHERE nutrition_food_id BETWEEN 510017 AND 510025
  AND (
      basis_unit <> 'g'
      OR review_status <> 'approved'
      OR source_name <> 'USDA FoodData Central'
      OR source_version !~ '^SR Legacy 2019-04-01 FDC-[0-9]+$'
      OR calories_kcal_per_100 < 0
      OR protein_g_per_100 < 0
      OR fat_g_per_100 < 0
      OR carbs_g_per_100 < 0
      OR is_deleted = TRUE
  );

SELECT conversion_id,
       nutrition_food_id,
       source_unit,
       target_unit,
       multiplier,
       source_name,
       source_version,
       review_status
FROM nutrition_unit_conversions
WHERE conversion_id BETWEEN 520017 AND 520025
ORDER BY conversion_id;

SELECT COUNT(*) AS common_unit_conversion_seed_rows
FROM nutrition_unit_conversions
WHERE conversion_id BETWEEN 520017 AND 520025
  AND is_deleted = FALSE;

SELECT COUNT(*) AS invalid_common_unit_conversion_seed_rows
FROM nutrition_unit_conversions
WHERE conversion_id BETWEEN 520017 AND 520025
  AND (
      nutrition_food_id NOT BETWEEN 510017 AND 510025
      OR target_unit <> 'g'
      OR multiplier <= 0
      OR source_name <> 'USDA FoodData Central API foodPortions'
      OR source_version !~ '^SR Legacy 2019-04-01 FDC-[0-9]+ portion-[0-9]+'
      OR review_status <> 'approved'
      OR is_deleted = TRUE
  );

SELECT COUNT(*) AS common_conversion_food_mismatch_rows
FROM nutrition_unit_conversions c
LEFT JOIN nutrition_foods f ON f.nutrition_food_id = c.nutrition_food_id
WHERE c.conversion_id BETWEEN 520017 AND 520025
  AND (f.nutrition_food_id IS NULL OR f.is_deleted = TRUE OR f.review_status <> 'approved');
