...
function HomeTaskAndInputStatesPanel({ nodeId }: { nodeId: '989:3' | '1015:248' }) {
  return (
    <section
      className={`${styles.statusPanel} ${styles.taskAndInputStatesPanel}`}
      aria-labelledby="task-and-input-states-title"
      data-node-id={nodeId}
      data-name="workspace::task-and-input-states"
    >
      <h2 id="task-and-input-states-title">任务入口与状态</h2>
      ...
      <div className={`${styles.page} ${isFigmaFixture ? styles.figmaHomePage : ''} fm-enter`}>
        ...
        {isRealMode && realLoading ? (
          ...
          <>
            ...
            {isFigmaFixture ? (
              <HomeTaskAndInputStatesPanel nodeId={homeState === 'input-states' ? '1015:248' : '989:3'} />
            ) : null}
          </>
        )}
      </div>
