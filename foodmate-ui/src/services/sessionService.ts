...
export async function loadSessions(): Promise<SessionSummary[]> {
  ...
  const body = await apiRequest<{ items: Array<{ session_id: string; title?: string; mode?: string; status?: string }>; total: number }>('/api/sessions?size=50');
...
export type RealSession = { session_id: string; user_id?: string; title: string; mode: string; status: string; last_message_at?: string };
export type RealMessage = { message_id: string; session_id: string; agent_run_id?: string; role: 'user' | 'assistant'; content: string; structured_payload?: string; sequence_no: number; created_at: string };
...
export async function searchSessions(query: string): Promise<SessionSummary[]> { const rows = await apiRequest<Array<{ session_id: string; title: string; snippet: string }>>(`/api/sessions/search?q=${encodeURIComponent(query)}`); return rows.map((row) => ({ id: row.session_id, title: row.title, subtitle: row.snippet })); }
