...
| M1-5 饮食业务 | 饮食记录创建、查询、编辑、删除、恢复，today/7d/30d 分析，餐食计划生命周期和购物清单已接入 Java/SQL/API；60 条 approved 营养 seed、60 条 USDA foodPortions 换算和 75 条精确质量换算已在本地 PostgreSQL 校验，matched/pending 分支保持可用。 |
...
| Agent、Eval 与 RAG | `run.eval_decided`、预算、checkpoint、continuation、追问和安全降级已进入运行路径；公共知识库已完成批量上传、异步索引、发布可见性和 `public_published` 安全引用。默认仍是 `deterministic:local`；宿主机和 Docker `agent-runtime` 均已用显式凭据验证 SiliconFlow Chat、`BAAI/bge-m3` 和 `Qwen/Qwen3-Embedding-0.6B` 的单次协议调用。两个 Embedding profile 使用独立 Milvus collection，当前容器恢复为 Qwen profile；长时间稳定性、正式价格审计和生产 RAG 治理仍未完成。 |
...
- 更大范围的营养目录和完整的业务 Tool/SQL 覆盖；当前本地目录扩展已达到 60 条 approved 食材。
...
- 2026-09-01 Docker `agent-runtime` smoke 已验证 `BAAI/bge-m3` 与 `Qwen/Qwen3-Embedding-0.6B` 均返回 1024 维向量，当前容器健康运行并恢复为 Qwen profile。切换到 BGE 必须使用 BGE 专用 collection、重新创建容器并重新索引，不能混写两个模型的向量；该结果是单次协议/业务证据，不代表长稳、容量或生产成本审计。
...
- M2-1 已在 Docker 应用容器中复验 `local-stub` Redis 索引和 local deterministic Milvus 路径，覆盖批次上传、RocketMQ 索引、结果回写、显式发布/下线/恢复、用户检索、AgentRun 引用和批次 SSE；D108 另行取得两个真实 SiliconFlow Embedding profile 的单次 Docker 协议证据。
