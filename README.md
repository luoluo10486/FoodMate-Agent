...
| Java 回归 | D182 Java 业务门禁为 Shared/Application/Infrastructure/API `12/257/119/72`，失败/错误 `0`，Infrastructure 条件跳过 `20`；本轮受影响文件的格式与架构约束未新增问题。HTTP 与 RocketMQ `food_log_writer` 回归各 11/11，包含官方 foodPortions 换算 matched/pending 数据库断言。具体运行批次和跳过项以 [`EXECUTION_RECORD.md`](./script/sql/FoodMate/EXECUTION_RECORD.md) 为准。 |
