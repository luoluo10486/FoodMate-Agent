...
| SQL Agent 饮食分析 | 已补齐今日热量、近 7 天蛋白质、指定食材出现次数、按实际餐次执行统计的计划完成度和按未勾选明细统计的购物缺项；缺少时间/食材、字段不支持和空结果均有明确澄清或安全空态，Java 继续执行 SQL Schema、用户范围、AST、LIMIT 和审计校验。 |
| M1-5 饮食业务 | 饮食记录创建、查询、编辑、删除、恢复，today/7d/30d 分析，餐食计划生命周期和购物清单已接入 Java/SQL/API；本地 PostgreSQL 当前有 1,000 条 approved/official USDA 食材和 1,518 条 approved USDA foodPortion 换算，规范键、来源 ID 和换算规则均通过校验，matched/pending 分支保持可用。饮食记录与复合菜现在共享中文单位归一化，候选界面展示每 100g 营养摘要。 |
...
| 前端 | G1-G6 页面代码边界、追问/确认/失败/取消/SSE 状态、真实管理查询和知识库批次/RAG 引用接入已完成；真实聊天历史会话现在会恢复最近 Run 并回放终态引用，管理端 R7 已补齐服务端分页和筛选。2026-09-08 已实际浏览复核饮食记录、摄入分析、餐食规划、知识库、记忆管理和管理概览，桌面关键控件可见，移动视口未发现页面级横向溢出。D182 前端全量业务门禁为 `46` 个测试文件、`306/306` 通过，typecheck 和 build 通过。 |
...
| Java 回归 | D182 Java 业务门禁为 Shared/Application/Infrastructure/API `12/257/119/72`，失败/错误 `0`，Infrastructure 条件跳过 `20`；D183 已完成目标 reactor `verify`，Spotless clean，Alibaba profile 的六个模块 Checkstyle 均为 `0 violations`。HTTP 与 RocketMQ `food_log_writer` 回归各 11/11，包含官方 foodPortions 换算 matched/pending 数据库断言。具体运行批次和跳过项以 [`EXECUTION_RECORD.md`](./script/sql/FoodMate/EXECUTION_RECORD.md) 为准。 |
...
- 营养目录的人工营养学复核和生产级目录治理仍未完成；复合菜基础闭环已完成，但当前数据库中的本轮复合菜测试记录已全部软删除，不能当作正式可用菜品。当前本地目录已导入 1,000 条可追溯 USDA 食材。
...
- 2026-09-08 D172 集中业务门禁：Java `foodmate-shared`/`foodmate-application`/`foodmate-infra`/`foodmate-api` 合计 `457` tests，失败/错误 `0`，Infrastructure 条件跳过 `20`；Python `246 passed、2 skipped、2 warnings、6 subtests passed`；前端 `46` 个测试文件、`303 passed`，typecheck、build 和 Docker Compose config 均通过。该结果只代表业务正确性，不代表性能或生产可靠性。
...
- 本地公共知识 RAG 的历史测试向量和 stub 索引已按保护脚本处理，当前公共 Milvus 可检索记录为 0；D167 的 6 个物理实体保留在集合中并统一标记为 `deleted=true`，对应 PostgreSQL 历史 chunk 保留但因文档软删除不可检索，Redis `foodmate:rag:stub:chunks` 为 0。Milvus 集合定义保留，便于后续重新导入正式知识；物理删除需走保留清理任务。
...
- Python 通过确定性规则提取喜欢、不喜欢/忌口、预算、烹饪能力、用餐时间和交互偏好，写入候选的只是短结构化值和用户消息来源 ID；一次性请求、计划、营养目标和高影响健康事实会被拦截。
...
- Python 记忆上下文测试 `61 passed`，Java Shared/Application/Infrastructure 定向测试和构建 `15/15` 通过；D162 使用真实 Java API + PostgreSQL 验证修改、过期、删除、上下文过滤和统一审计。本轮未调用真实付费模型/Embedding，未执行故障/性能测试。

## 2026-09-06 K5 SQL Agent 饮食分析覆盖

- SQL Planner 现在支持今日热量、近 7 天蛋白质、指定食材出现次数、基于稳定计划餐次与有效饮食记录的实际执行完成度和基于未勾选明细的购物缺项统计；指定早餐、午餐或晚餐时，食材次数统计会保留餐次过滤。
