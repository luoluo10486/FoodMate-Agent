...
- `待办任务TODO.md` 是执行路线图。
- Phase 1 前端静态 UI 原型已在 `frontend/` 中创建；当前仍不创建真实后端、数据库或 Agent 运行时。
...
Phase 1 静态 UI 原型已开始落地，前端技术栈为 React + Vite + TypeScript + Arco Design。下一步推荐继续补齐 Phase 1 细节并进入 Phase 2 Mock 交互。
...
第一轮运行时实现应先完成前端静态 UI、mock 交互、个人资料和管理后台入口，再进入后端认证、个人资料、会话与 Agent 主链路。
