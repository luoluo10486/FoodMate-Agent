...
> **公共知识索引状态口径**：D134 的 Docker 真实 Embedding + Milvus + Chat 闭环是历史隔离批次的一次性业务证据；R5 的 9 份正式资料只完成了隔离的 `local-stub` 内容和切分验收。当前正式公共资料尚未重新写入 PostgreSQL/Redis/Milvus，真实 Embedding 重建仍等待用户明确授权。这里的公共知识状态不影响已经存在的 Java/Python 代码、营养目录独立向量索引或 `local-stub` 业务测试。
...
| Agent、Eval 与 RAG | `run.eval_decided`、预算、checkpoint、continuation、追问和安全降级已进入运行路径；公共知识库的批量上传、异步索引、发布可见性和 `public_published` 安全引用代码已完成。R5 用 9 份正式资料完成主题覆盖审计和隔离 `local-stub` 验收，生成 50 个 chunk；当前正式公共资料尚未重建 PostgreSQL/Redis/Milvus 索引，真实 Embedding 等待授权。D134 的 Docker 真实 Embedding + Milvus + Chat AgentRun 引用闭环保留为历史隔离批次证据，默认仍是 `deterministic:local`；长稳、正式价格审计和生产 RAG 治理仍未完成。 |
