...
export function DeletedSection({ onAction }: { onAction: (payload: AdminActionPayload) => void }) {
  ...
  const deletedColumns: TableColumnProps<DeletedRow>[] = [
    ...
    { title: '原因', dataIndex: 'reason' },
    {
      ...
      render: (_, record) => (
        <Button size="mini"
          onClick={() => onAction({ action: '恢复软删除资源', targetLabel: `${record.resourceType}:${record.resourceId}`, targetType: record.resourceType, targetId: record.resourceId, onApply: () => { const rowIndex = adminDeletedRows.findIndex((item) => item.key === record.key); if (rowIndex >= 0) adminDeletedRows.splice(rowIndex, 1); } })}>恢复</Button>
      )
    }
  ];
    ...
    <>
      ...
      <Card className={styles.wideCard} bordered={false}>
        <div className={styles.cardHead}>
          <strong>软删除资源</strong>
          <Button disabled={!canManage} size="mini" color="red">恢复仅 admin 可用</Button>
        </div>
