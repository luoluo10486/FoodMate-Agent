...
export function KnowledgeSection({ onAction }: { onAction: (payload: AdminActionPayload) => void }) {
    ...
    { title: '更新时间', dataIndex: 'updatedAt' },
    {
      ...
      render: (_, record) => (
        <div className={styles.rowActions}>
          <Button size="mini" onClick={() => setSelectedDoc(record)}>详情</Button>
          <Button size="mini" disabled={!canManage}
            onClick={() => onAction({ action: record.status === 'indexed' ? '下线文档' : '恢复文档', targetLabel: record.documentId, targetType: 'knowledge_document', targetId: record.documentId, onApply: () => { record.status = record.status === 'indexed' ? 'disabled' : 'indexed'; record.indexProgress = record.status === 'indexed' ? '100%' : '0%'; } })}>
  ...
  return (
    <>
      ...
      <section className={styles.sectionLayout}>
        <Card className={styles.wideCard} bordered={false}>
          <div className={styles.cardHead}>
            <strong>知识库文档</strong>
            <Button type="primary" disabled={!canManage} onClick={() => setUploadVisible(true)}>上传文档</Button>
          </div>
      ...
      <Modal title="上传知识库文档" visible={uploadVisible} okText="提交 mock 上传" cancelText="取消"
        onCancel={() => setUploadVisible(false)}
        onOk={() => { setUploadVisible(false); Message.success('文档上传已提交；真实接入会写 MinIO 对象和索引任务'); }}>
  ...
  return (
    <Card className={styles.card} bordered={false}>
      ...
      <div className={styles.detailGrid}>
        <span>文档 ID</span><strong>{document.documentId}</strong>
        <span>来源</span><strong>{document.source}</strong>
        <span>索引进度</span><strong>{document.indexProgress}</strong>
        <span>切片数</span><strong>{document.chunks}</strong>
      </div>
    </Card>
  );
}
