 ...
 *
 * <p>把 RocketMQ 的消费回调翻译成与传输无关的 {@link MqMessageHandler}， 让 application 层能在自己的事务里处理消息而不依赖 MQ 客户端类型。
 *
 * <p>顺序消费用于 Agent 事件：Python 按 {@code run_id} 选择队列，同一 Run 的事件落在同一队列， 顺序消费保证 {@code event_seq} 按序进入
 * Inbox，把「等待前序事件」的重试降到最少。 {@link MqConsumeDecision#RETRY} 会挂起当前队列，因此该队列上后续 Run 的事件也会等待； 这是保序的代价，通过
 * {@code maxReconsumeTimes} 触发 DLQ 兜底避免永久阻塞。
 */
public final class RocketMqConsumerContainer implements AutoCloseable {
    ...
    /**
     * handler 抛出未受检异常时按可重试处理：这类异常通常来自数据库或网络， 确定性错误应由 handler 自己返回 {@link MqConsumeDecision#REJECT}。
     */
