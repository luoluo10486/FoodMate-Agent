...
对应接口文档：[接口与数据规范](./接口与数据规范.md)
文档边界：本文件只定义 Agent 角色、Prompt 协议、工具协议与调用策略；Workflow 节点、在线 Eval Gate、循环预算与退回规则以[Agent 运行架构](../架构/Agent运行架构.md)为唯一依据，最终架构分层、工具优先级、查询主路径和模型治理边界以总设计文档为准。
...
SQL Guard、用户过滤、敏感字段控制、数据源凭据、只读执行和审计全部属于 Java。Python 不直接执行 SQL。
...
营养值只能由 Java 根据 PostgreSQL `nutrition_foods` 和单位换算规则返回；模型不得临时估算或修改这些数值。无法匹配或换算时返回 `pending`/不完整结果，不伪造完整营养结果。

### 8.4 food_log_writer

用途：

- 提交饮食记录写入提案；最终写入由 Java application 在确认、幂等、revision、营养匹配和审计校验后执行
- `operation` 只允许 `create`、`update`、`delete`、`restore`。`create` 不绑定已有资源；其他操作必须提供 `resource_id` 和当前 `revision`
- Python 只能提交 Proposal，不能自行 confirm、reject 或执行；Java Tool Gateway 是工具名、Proposal type、用户归属和状态映射的权威
...
`update` 的输入沿用 `create` 的 `meal_time`、`items`、`notes`，并增加 `resource_id` 和 `revision`；`delete`/`restore` 只需要 `resource_id` 和 `revision`。所有操作仍必须携带 Proposal 的 `confirmation_ref` 和执行幂等键。
...
Tool Gateway 的确认结果在写入前可能为：

| Result status | error code | 含义 |
|---|---|---|
| `confirmation_required` | `TOOL_CONFIRMATION_REQUIRED` / `TOOL_CONFIRMATION_EXPIRED` | 尚未确认或确认已过期 |
| `rejected` | `TOOL_CONFIRMATION_REJECTED` | 用户拒绝该写入提案 |
| `superseded` | `TOOL_CONFIRMATION_SUPERSEDED` | 同一资源和操作已有更新提案取代该确认 |
| `failed` | `TOOL_EXECUTION_FAILED` / `TOOL_IDEMPOTENCY_CONFLICT` | 业务写入失败或幂等键冲突 |

`success` 只在 Java 事务提交后返回；`failed` 不得被解释为已写入。相同 Proposal 重放必须返回稳定的状态和错误码，不得再次执行写副作用。当前 `food_log_writer` 的 create/update/delete/restore 以及 rejected/failed/superseded、revision 冲突和幂等重放均已通过真实 HTTP/MQ 跨进程回归。
...
- 使用了默认份量
- 预算未知时采用了保守估计
- 数据缺失时保留原始记录并明确标记不完整，不使用模型近似营养值
