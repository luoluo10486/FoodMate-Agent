...
`success` 只在 Java 事务提交后返回；`failed` 不得被解释为已写入。相同 Proposal 重放必须返回稳定的状态和错误码，不得再次执行写副作用。当前 `food_log.create` 已有真实 HTTP/MQ 跨进程回归；update/delete/restore 及上述新增状态仍需补充跨进程回归。
