...
> 当前口径（2026-09-02）：本地 API 已实现饮食记录创建/查询/编辑/删除/恢复、today/7d/30d 营养分析、餐食计划创建/查询/修改/校验/保存/删除/恢复/购物清单，以及 `meal_plan.save_plan` Proposal -> Confirm -> Execute。`food_log_writer` 的 create/update/delete/restore 已实现本地 Tool Gateway/Runtime Proposal，必须绑定 `confirmation_ref`，并校验 AgentRun/用户归属、参数摘要、revision 和幂等键；rejected/failed/superseded、事务回滚、失败审计和重放保护已通过真实 PostgreSQL HTTP/RocketMQ 各 11/11 跨进程回归。M2-1 公共知识库已实现批次上传、PDF/DOCX/Markdown/TXT 安全解析、异步索引结果回写、发布/下线/删除/恢复、批次 SSE 和 `public_published` 引用；M2-2 的 Tool/SQL 本地业务闭环、M2-3 管理查询/写操作和 M3 可审计任务切片已有业务测试证据。计划资源生命周期已通过 Java 定向测试和本地 HTTP 回归；营养目录当前为 60 条 approved USDA 食材 seed、60 条 approved foodPortion 换算规则和 75 条精确质量换算，V8 seed/validation 已通过本地验证。编辑接口为 `PATCH /api/food-logs/{foodLogId}?revision=...`，必须携带 `Idempotency-Key`，采用整条可编辑内容替换和 `revision` CAS。`food_logs` 使用 `food_log_items` 明细，不再使用 `items_json/nutrition_json`；营养目录由数据库表提供，计算使用 Java `BigDecimal`。本节同时保留目标契约，未实现接口和生产级门禁不得视为已开放。

> **Java/Python 内部 wire 契约警告**：本文历史章节中的 Agent 内部 Run、Cancel、Tool、SQL 和错误格式只保留概念说明，不得用于实现、Schema/DTO 生成、fixture 或契约测试。内部消息 envelope、canonical digest、幂等、版本和错误语义由[双运行时内部契约 V1](./双运行时内部契约V1.md)唯一权威定义；RocketMQ Topic、ACK、顺序与 DLQ 绑定见其第 16 节和[ADR-0005](../决策/ADR-0005-RocketMQ异步主通道.md)。HTTP 仅保留为 M1-3 兼容/测试适配。
...
- Java/Python 内部消息：传输无关版本化 envelope；目标正式通道为 RocketMQ，HTTP 为兼容/测试适配；wire 细节只见[双运行时内部契约 V1](./双运行时内部契约V1.md)
- 内部任务：需要可靠异步后再引入队列，MVP 不预设 Kafka

### 2.3 双运行时信任边界

- Java 从认证结果派生用户和 scope；Python 回传的身份字段不作为授权依据。
...
| 方法 | 路径 | 用途 | 鉴权 |
|---|---|---|---|
| `POST` | `/api/auth/login` | 登录并设置会话、CSRF 和 HttpOnly Refresh Cookie；不在 JSON 返回明文 token | 匿名 |
| `POST` | `/api/auth/refresh` | 使用 HttpOnly Refresh Cookie 原子轮换 token，并重新设置认证 Cookie | 匿名（同源） |
| `POST` | `/api/auth/logout` | 撤销当前会话和所提交的 Refresh Token | 用户 |
| `GET` | `/api/users/me` | 查询当前用户、角色和资料摘要 | 用户 |
| `POST` | `/api/auth/register` | 注册普通用户并设置认证 Cookie | 匿名 |
| `POST` | `/api/auth/password-reset/request` | 发起密码找回 | 匿名 |
| `POST` | `/api/auth/password-reset/confirm` | 确认密码重置并撤销全部认证会话 | 匿名 |
| `GET` | `/api/users/me/profile` | 查询个人资料 | 用户 |
| `PUT` | `/api/users/me/profile` | 修改昵称、展示名和营养偏好 | 用户 |
| `GET` | `/api/users/me/avatar` | 获取当前头像 | 用户 |
| `POST` | `/api/users/me/avatar` | 上传或替换头像 | 用户 |
| `DELETE` | `/api/users/me/avatar` | 删除头像 | 用户 |
...
- 服务端必须按文件实际字节校验签名、像素上限和解码结果；原始文件名不得参与对象 key 生成。
- 头像业务响应只返回稳定 `avatar_url`、MIME、大小和资产 ID，不返回对象存储 key；GET 接口仅返回短时签名重定向。
...
| run.failed | 失败 |
| run.cancelled | 已取消 |
| run.superseded | 旧 Run 已由 continuation Run 接续，终态事件；payload 携带 `superseded_by_run_id` |
...
> 单文件上传接口保留用于兼容性适配和小文件即时提交；管理端批量上传不得在 HTTP 请求内同步完成解析、切分、Embedding 或向量索引写入。正式管理端上传使用下面的异步批次接口。

#### 5.4.4 批量上传与异步索引

正式管理端路径为 `POST /api/admin/knowledge-documents/upload-batches`；下列 `/foodmate` 路径是早期目标契约，保留用于历史对照。

权限：`admin/superadmin`。请求使用 `multipart/form-data`，字段如下：

- `files[]`：一个或多个文档文件；单文件上限 20 MB，单批次最多 20 个文件。
- `source_type`：`admin_upload`、`internal_sop`、`original_recipe`、`public_reuse` 或 `policy`。
- `source_name`、`source_version`、`license_notice`：必填来源、版本和授权说明。
- `idempotency_key`：客户端生成的批次幂等键；同一操作者重放返回同一批次。HTTP `Idempotency-Key` 不是当前 multipart 控制器的替代字段。

接口只负责校验、接收文件并创建任务，当前响应包装为 `200` 并返回 `batch_id/status`；不得等待解析、切分、Embedding 或 Milvus 写入完成。Worker 通过 Java Outbox 异步处理索引。
...
```json
{
  "success": true,
  "data": {
    "batch_id": "1912345678901234701",
    "status": "queued",
    "accepted": [
      {
        "document_id": "1912345678901234702",
    ...
    "rejected": [],
    "links": {
      "status": "/foodmate/admin/knowledge-upload-batches/1912345678901234701",
      "events": "/foodmate/admin/knowledge-upload-batches/1912345678901234701/events"
    }
  }
}
```
...
`queued -> uploading -> uploaded -> indexing -> completed`

异常终态为 `partial_failed`、`failed` 或 `cancelled`。每个文档同时保存独立的 `upload_status` 和 `index_status`；文件上传成功不代表向量索引成功。单文件失败不得回滚同批次已经成功接收的其他文件。

#### 5.4.5 查询上传批次

正式管理端路径为 `GET /api/admin/knowledge-upload-batches/{batchId}`。

返回批次汇总、每个文件的上传/索引状态、当前阶段、已完成数量、失败数量、进度百分比、错误码和可重试动作。管理端离开页面后可通过该接口恢复任务状态。

#### 5.4.6 上传批次事件

正式管理端路径为 `GET /api/admin/knowledge-upload-batches/{batchId}/events`。

使用 `text/event-stream` 返回 `knowledge.upload.progress`、`knowledge.document.indexing`、`knowledge.document.completed`、`knowledge.document.failed` 和 `knowledge.batch.completed` 事件。事件至少包含 `batch_id`、`document_id`、`status`、`completed_count`、`total_count`、`progress`、`error_code`、`request_id` 和 `trace_id`。

#### 5.4.7 重试失败文档

正式管理端路径为 `POST /api/admin/knowledge-upload-batches/{batchId}/documents/{documentId}/retry`。

只允许重试 `index_failed` 文档，不得重复创建文档版本；服务端重置该条目的索引状态、清理旧结果去重事实并生成新的索引任务。重试操作当前返回 `200`，前端继续通过批次状态接口或 SSE 展示进度。
...
汇总必须同时返回 `matched_item_count`、`total_item_count`、`coverage` 和 `incomplete=true/false`。支持的范围为 `day`、`7d`、`30d`；目标来自用户 Profile，未设置目标时只返回实际摄入，不判断达标。
...
Agent/自然语言写入使用独立确认事实：

- 已实现本地兼容路径为 `POST /api/approvals/proposals`：创建待确认提案，当前支持 `meal_plan.save_plan` 和 `food_log.create`；`food_log_writer` 通过内部 Proposal 携带 `confirmation_ref` 执行同一确认事实。
- 已实现本地兼容路径为 `POST /api/approvals/{approval_request_id}/confirm`：确认提案。
- 已实现本地兼容路径为 `POST /api/approvals/{approval_request_id}/execute`：执行已确认的保存计划或饮食记录创建；重放返回已执行状态，不重复产生业务写入。`food_log_writer` 的真实 HTTP/MQ 回归已通过，覆盖确认绑定、营养匹配、资源 ID 回填和重放保护。
- 兼容路径暂以 `/api/approvals/*` 为准；Proposal 的拒绝、失败、superseded、事务回滚、失败审计和重放保护已在本地业务测试及 `food_log_writer` HTTP/RocketMQ 回归中覆盖。早期 `/foodmate/approval-requests` 目标路径仍未作为独立外部路由开放。

确认必须绑定当前用户、资源、操作、参数摘要和有效期。当前 `meal_plan.save_plan` 已验证参数变化、过期、已执行重放和 CAS 执行；`food_log_writer` 的 create/update/delete/restore 已在 application/Tool Gateway 以及真实 HTTP/MQ 跨进程回归中验证 AgentRun/用户归属、幂等键、参数摘要、revision、资源 ID 回填和重放保护。参数变化、过期、已执行或已被新提案替代时，旧确认不得执行。确认、业务写入和操作审计必须在同一事务完成；失败审计使用独立失败事务。
...
第一版限制为单用户、1-7 天、早餐/午餐/晚餐；支持人数、预算、热量目标、蛋白质目标、忌口和过敏原。计划状态固定为 `draft -> validated -> saved`；修改后回到 `draft`，只有 `saved` 计划才能生成正式购物清单。
...
| failed | 已失败 |
| cancelled | 已取消 |
| superseded | 已启用终态：旧 Run 已由后续补充消息创建的 continuation Run 接续；由 V5、Java 状态机和前端映射支持 |
...
- 基线索引：
  ...
  - food_log_items(food_log_id, item_order, is_deleted)
  - nutrition_foods(canonical_name, review_status, is_deleted)
...
M1-5 写入进一步要求：相同幂等键且参数摘要相同返回首次结果；参数摘要不同返回幂等冲突。`revision` 不一致返回资源冲突，不覆盖他人修改。营养计算不接受客户端传入的可信汇总。
...
Java 业务控制面保持模块化单体，Python Agent Runtime 是独立部署单元。当前 Python 工程、M1-3 HTTP stub、M1-4 RocketMQ/Redis 基础传输、固定 Workflow、模型适配、Eval、checkpoint、Proposal/Result、公共知识库 RAG 和 Tool/SQL 本地业务闭环已创建并验证；真实云模型/embedding 长稳、生产 Trace/指标、生产记忆治理和容量/故障门禁仍未完成。旧 Java Agent 模块不得恢复为实现目标。
