...
> M1-5 当前口径（2026-08-13）：第一切片已实现本地 API：饮食记录创建/查询/编辑/删除/恢复、today/7d/30d 营养分析、餐食计划创建/校验/保存/购物清单，以及 `meal_plan.save_plan` Proposal -> Confirm -> Execute。完整计划资源 CRUD、营养目录 seed、完整 `food_log_writer` Tool Gateway 和更多确认状态仍未完成。编辑接口为 `PATCH /api/food-logs/{foodLogId}?revision=...`，必须携带 `Idempotency-Key`，采用整条可编辑内容替换和 `revision` CAS。`food_logs` 使用 `food_log_items` 明细，不再使用 `items_json/nutrition_json`；营养目录由数据库表提供，当前为空，计算使用 Java `BigDecimal`。本节同时保留目标契约，未实现接口不得视为已开放。
...
- 已实现本地兼容路径为 `POST /api/approvals/proposals`：创建待确认提案，当前仅支持 `meal_plan.save_plan`。
- 已实现本地兼容路径为 `POST /api/approvals/{approval_request_id}/confirm`：确认提案。
- 已实现本地兼容路径为 `POST /api/approvals/{approval_request_id}/execute`：执行已确认的保存计划；重放返回已执行状态，不重复保存。
- 目标路径 `POST /foodmate/approval-requests`、拒绝接口、完整 failed/superseded 状态仍待实现。

确认必须绑定当前用户、资源、操作、参数摘要和有效期。当前 `meal_plan.save_plan` 已验证参数变化、过期、已执行重放和 CAS 执行；参数变化、过期、已执行或已被新提案替代时，旧确认不得执行。确认、业务写入和操作审计必须在同一事务完成。
