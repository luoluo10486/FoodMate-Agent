...
| failed | 已失败 |
| cancelled | 已取消 |
| superseded | 目标终态：已由后续补充消息创建的新 Run 接续；当前 V1 不得返回，需后续契约和迁移启用 |
...
Java 业务控制面保持模块化单体，Python Agent Runtime 是独立部署单元。当前 Python 工程和 M1-3 HTTP stub 已创建；M1-4 RocketMQ、LangGraph、真实模型与 Eval 尚未实现。旧 Java Agent 模块不得恢复为实现目标。
