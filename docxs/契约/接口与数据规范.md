...
> M1-5 当前口径（2026-08-12）：饮食记录、营养分析、餐食计划和写确认已完成决策但尚未实现。`food_logs` 使用 `food_log_items` 明细，不再使用 `items_json/nutrition_json`；营养目录由数据库表提供，计算使用 Java `BigDecimal`。本节接口是实施契约，不代表代码已完成。
...
- `POST /foodmate/approval-requests`：创建待确认提案，返回参数摘要和过期时间。
- `POST /foodmate/approval-requests/{approval_request_id}/confirm`：确认提案。
- `POST /foodmate/approval-requests/{approval_request_id}/reject`：拒绝提案。

确认必须绑定当前用户、资源、操作、参数摘要和有效期。参数变化、过期、已执行或已被新提案替代时，旧确认不得执行。确认、业务写入和操作审计必须在同一事务完成。
