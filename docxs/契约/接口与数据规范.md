...
> M1-5 当前口径（2026-08-14）：第一切片已实现本地 API：饮食记录创建/查询/编辑/删除/恢复、today/7d/30d 营养分析、餐食计划创建/查询/修改/校验/保存/删除/恢复/购物清单，以及 `meal_plan.save_plan` Proposal -> Confirm -> Execute。`food_log_writer` 的 `food_log.create` 已实现本地 Tool Gateway/Runtime Proposal 第一切片，必须绑定 `confirmation_ref`，并校验 AgentRun/用户归属、参数摘要和幂等键；真实 PostgreSQL HTTP/RocketMQ writer 回归已通过，完整 Tool Gateway 和更多确认状态仍未完成。计划资源生命周期已通过 Java 定向测试和本地 HTTP 回归；营养目录 V1 已导入 5 条 approved USDA seed，单位换算暂为空。编辑接口为 `PATCH /api/food-logs/{foodLogId}?revision=...`，必须携带 `Idempotency-Key`，采用整条可编辑内容替换和 `revision` CAS。`food_logs` 使用 `food_log_items` 明细，不再使用 `items_json/nutrition_json`；营养目录由数据库表提供，计算使用 Java `BigDecimal`。本节同时保留目标契约，未实现接口不得视为已开放。
...
确认必须绑定当前用户、资源、操作、参数摘要和有效期。当前 `meal_plan.save_plan` 已验证参数变化、过期、已执行重放和 CAS 执行；`food_log.create` 已在 application/Tool Gateway 定向测试中验证 AgentRun/用户归属、幂等键、参数摘要、资源 ID 回填和重放保护。参数变化、过期、已执行或已被新提案替代时，旧确认不得执行。确认、业务写入和操作审计必须在同一事务完成；拒绝/失败/superseded 状态仍待完整实现。
