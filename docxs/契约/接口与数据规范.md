...
Java 业务控制面保持模块化单体，Python Agent Runtime 是独立部署单元。当前 Python 工程、M1-3 HTTP stub 和 M1-4 RocketMQ/Redis 基础传输闭环已创建并验证；LangGraph、真实模型、Eval 与完整记忆治理尚未实现。旧 Java Agent 模块不得恢复为实现目标。
