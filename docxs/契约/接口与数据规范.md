...
> M1-5 当前口径（2026-08-13）：第一切片已实现本地 API：饮食记录创建/查询/删除/恢复、today/7d/30d 营养分析、餐食计划创建/校验/保存/购物清单，以及 `meal_plan.save_plan` Proposal -> Confirm -> Execute。编辑、完整计划资源 CRUD、营养目录 seed、完整 `food_log_writer` Tool Gateway 和更多确认状态仍未完成。`food_logs` 使用 `food_log_items` 明细，不再使用 `items_json/nutrition_json`；营养目录由数据库表提供，当前为空，计算使用 Java `BigDecimal`。本节同时保留目标契约，未实现接口不得视为已开放。
