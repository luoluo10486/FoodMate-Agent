...
> M1-5 当前口径（2026-08-14）：第一切片已实现本地 API：饮食记录创建/查询/编辑/删除/恢复、today/7d/30d 营养分析、餐食计划创建/查询/修改/校验/保存/删除/恢复/购物清单，以及 `meal_plan.save_plan` Proposal -> Confirm -> Execute。`food_log_writer` 的 `food_log.create` 已实现本地 Tool Gateway/Runtime Proposal 第一切片，必须绑定 `confirmation_ref`，并校验 AgentRun/用户归属、参数摘要和幂等键；真实 PostgreSQL HTTP/RocketMQ writer 回归、完整 Tool Gateway 和更多确认状态仍未完成。计划资源生命周期已通过 Java 定向测试和本地 HTTP 回归；营养目录 seed 仍未完成。编辑接口为 `PATCH /api/food-logs/{foodLogId}?revision=...`，必须携带 `Idempotency-Key`，采用整条可编辑内容替换和 `revision` CAS。`food_logs` 使用 `food_log_items` 明细，不再使用 `items_json/nutrition_json`；营养目录由数据库表提供，当前为空，计算使用 Java `BigDecimal`。本节同时保留目标契约，未实现接口不得视为已开放。
...
- 已实现本地兼容路径为 `POST /api/approvals/{approval_request_id}/execute`：执行已确认的保存计划或饮食记录创建；重放返回已执行状态，不重复产生业务写入。`food_log_writer` 真实 HTTP/MQ 回归仍待完成。
