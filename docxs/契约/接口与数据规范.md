...
`POST /foodmate/admin/knowledge-documents/upload-batches`

权限：`admin`。请求使用 `multipart/form-data`，字段如下：

- `files[]`：一个或多个文档文件；单文件上限 20 MB，单批次最多 50 个文件、总大小不超过 200 MB。
- `source_type`：`sop`、`recipe`、`nutrition_manual` 或 `other`。
- `visibility`：`private`、`tenant` 或 `public`，默认 `tenant`。
- `start_indexing`：是否在上传完成后自动开始索引，默认 `true`。
- `Idempotency-Key`：客户端生成的幂等键，重复提交返回同一批次结果。

接口只负责校验、接收文件并创建任务，必须在文件接收完成后立即返回 `202 Accepted`，不得等待解析、切分、Embedding 或 Milvus 写入完成。
...
`GET /foodmate/admin/knowledge-upload-batches/{batch_id}`

返回批次汇总、每个文件的上传/索引状态、当前阶段、已完成数量、失败数量、进度百分比、错误码和可重试动作。管理端离开页面后可通过该接口恢复任务状态。

#### 5.4.6 上传批次事件

`GET /foodmate/admin/knowledge-upload-batches/{batch_id}/events`
...
`POST /foodmate/admin/knowledge-upload-batches/{batch_id}/documents/{document_id}/retry`

只允许重试 `failed` 文档，不得重复创建文档版本；服务端生成新的索引任务并将该文档的 `index_status` 置为 `queued`。重试操作返回 `202 Accepted`，前端继续通过批次状态接口或 SSE 展示进度。
...
- 目标路径 `POST /foodmate/approval-requests`、拒绝接口、完整 failed/superseded 状态仍待实现。
...
Java 业务控制面保持模块化单体，Python Agent Runtime 是独立部署单元。当前 Python 工程、M1-3 HTTP stub、M1-4 RocketMQ/Redis 基础传输、固定 Workflow、模型适配、Eval、checkpoint 和 Proposal/Result 闭环已创建并验证；生产 RAG、完整业务 Tool/SQL、云模型长稳和生产记忆治理仍未完成。旧 Java Agent 模块不得恢复为实现目标。
