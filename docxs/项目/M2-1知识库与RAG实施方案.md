...
## 2026-09-06 K2 知识切分与检索质量收口

`chunk_markdown` 已从“按最近一级标题和固定窗口”调整为统一的章节/段落切分：标题层级生成 `section_path`，同章节的连续段落优先合并，长段落优先在句末、标点或空白处拆分，默认目标为 `700` 字符、硬上限 `1000` 字符、相邻切片重叠 `80` 字符。保留既有 `document_id + version + sequence` 的稳定 `embedding_id` 生成规则，并兼容旧调用方传入较小 `max_chars` 的调用方式。
