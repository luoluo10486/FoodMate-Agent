...
> 2026-08-23 状态复核：M2-1/M2-2/M2-3 的业务功能、权限边界和核心业务测试已完成；M2-1 的 Docker deterministic stub/local Milvus 业务闭环也已实际验证。M3 的运营审计、DLQ 重放契约、保留治理和受控清理任务代码切片已完成。不可逆数据库硬删除、真实依赖清理、性能/故障矩阵、生产部署和安全演练仍是后置范围。

## MVP 最短路径（做完这些就能跑）

> **当前进度：M1-2、M1-3 已完成，M1-4 本地 Agent 闭环已完成，M1-5 核心业务和写确认跨进程回归已完成本地验收。当前已验证固定 Workflow、受控模型适配、预算、Eval Gate、checkpoint、Proposal/Result、浏览器 SSE、饮食记录基础读写、5 条营养目录 seed 的匹配、5 条 USDA foodPortions 单位换算规则、营养分析、餐食计划完整资源生命周期、`meal_plan.save_plan` 写确认，以及 `food_log_writer` 在真实 HTTP/RocketMQ 下各 11/11 的 create、拒绝、失败、superseded、update/delete/restore、revision 冲突和幂等重放；默认仍为 `deterministic:local`。更广泛营养目录、生产长压、云模型长稳、价格/账单审计和生产 Eval 治理仍在后续范围。下方 B 编号任务保留为历史拆分清单，状态必须以本文件顶部和 M1-5 实施方案为准。**
...
状态：映射结构已补齐（V1 表结构已完成）；用户、资料、会话和消息已完成本地 PostgreSQL 真实读写回归，刷新令牌和头像业务写路径仍待接入验收。
