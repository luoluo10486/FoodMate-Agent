...
| 4 | M1-5 | 饮食业务、写确认与最小 Java 工具 | 第一切片已完成，整体未完成 | 先补目录/编辑，再补完整 Tool Gateway 和确认状态 |
| 5 | M1-6 | 本地审计、metrics、压测与故障恢复 | 范围已收窄，代码待实现 | 生产监控、部署、备份和回滚后置 |

> **当前已可验证登录 → 对话 → AgentRun → Java Outbox → RocketMQ → Python Redis Inbox → 固定 Workflow/Eval → RocketMQ event → Java PostgreSQL Inbox/SSE，以及独立本地 HTTP 的饮食记录、营养分析、餐食计划和 `meal_plan.save_plan` 写确认切片。M1-5 尚未整体完成；真实营养目录、编辑、完整业务 Tool Gateway、拒绝/失败状态和本地 M1-6 门禁仍未完成。**
