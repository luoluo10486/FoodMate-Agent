...
| 4 | M1-5 | 饮食业务、写确认与最小 Java 工具 | 核心范围已完成，扩展持续 | 饮食记录、营养分析、计划生命周期、完整 `food_log_writer` 写操作、60 条官方 foodPortions 换算、75 条精确质量换算和 HTTP/MQ 各 11/11 跨进程回归已完成；生产治理仍后置 |
| 5 | M1-6 | 本地审计、metrics、压测与故障恢复 | 审计/指标代码已完成验证，整体未完成 | 双 JVM/Java 重启已有本机基线；共享依赖业务流量、完整队列与故障矩阵仍待实际证据 |

> **当前已可验证登录 → 对话 → AgentRun → Java Outbox → RocketMQ → Python Redis Inbox → 固定 Workflow/Eval → RocketMQ event → Java PostgreSQL Inbox/SSE，以及独立本地 HTTP 的饮食记录创建/查询/编辑/删除/恢复、60 条目录 matched/pending 营养分析、60 条官方 foodPortions 换算、75 条精确质量换算、餐食计划完整资源生命周期和 `meal_plan.save_plan` 写确认；`food_log_writer` 的 11 个 HTTP 和 11 个 RocketMQ 跨进程回归均已通过，包含确认绑定、营养匹配、拒绝/失败/superseded、update/delete/restore、revision 冲突和重放保护。另已完成本机两个独立 Java JVM 共享 PostgreSQL 的有界读取基线和 Java 重启后回读；M1-6 仍未整体完成。**
