...
> 2026-09-05 状态复核：M2-1/M2-2/M2-3 的业务功能、权限边界和核心业务测试已完成；M2-1 的 Docker PostgreSQL/Redis/RocketMQ/MinIO/Milvus 公共知识库闭环已完成真实 Embedding、索引结果回写、显式发布、AgentRun 引用和 SSE 回放验证；M2-2 已完成真实 SiliconFlow Chat 驱动 SQL Planner、只读 SQL Guard、查询审计和 `run.completed` 闭环。Docker `agent-runtime` 的启动/readiness、Qwen3 Embedding 1024 维真实请求和 DeepSeek-V4-Flash Chat 真实请求均有本机证据。M1-4 已增加结构化 Agent 反馈入口、记忆候选三层数据边界和业务测试，M3 的运营审计、DLQ 重放契约、保留治理和受控清理任务代码切片已完成；calculator、plan_validator 的 Java/Python 工具路径及统一工具执行审计已完成并通过定向业务测试。M1-5 营养目录当前含 60 条 approved 食材、60 条 foodPortions 换算和 75 条精确质量换算，V8 seed/validation 已在本地 PostgreSQL 执行通过。不可逆数据库硬删除、真实依赖清理、性能/故障矩阵、生产部署和安全演练仍是后置范围。

## MVP 最短路径（做完这些就能跑）

> **当前进度：M1-2、M1-3 已完成，M1-4 本地 Agent 闭环与结构化反馈业务入口已完成，M1-5 核心业务和写确认跨进程回归已完成本地验收。当前已验证固定 Workflow、受控模型适配、预算、Eval Gate、checkpoint、Proposal/Result、浏览器 SSE、结构化反馈的归属/幂等/审计边界、饮食记录基础读写、60 条营养目录 seed 的匹配、60 条 USDA foodPortions 单位换算规则、75 条精确质量换算、营养分析、餐食计划完整资源生命周期、`meal_plan.save_plan` 写确认，以及 `food_log_writer` 在真实 HTTP/RocketMQ 下各 11/11 的 create、拒绝、失败、superseded、update/delete/restore、revision 冲突和幂等重放；M2-1 公共知识库真实 Embedding/Milvus 引用链路和 M2-2 真实云 SQL Agent 只读链路也已取得本机业务证据。更大范围营养目录、生产长压、云模型长稳、价格/账单审计和生产 Eval 治理仍在后续范围。下方 B 编号任务保留为历史拆分清单，状态必须以本文件顶部和 M1-5 实施方案为准。**
