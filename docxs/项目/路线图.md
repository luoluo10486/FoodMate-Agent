...
| 范围 | 状态 | 当前边界 |
|---|---|---|
| M2-1 公共知识库与 RAG | 核心代码与业务定向测试已完成，真实闭环未完成 | Java/Python 索引、stub/local 检索、可见性和引用契约、知识库用户检索、管理端批次上传/进度/重试、聊天可展开引用、餐食规划列表/详情/创建/购物清单 real 接入已验证；上传、索引、发布、检索、引用的真实跨运行时验证仍待完成 |
