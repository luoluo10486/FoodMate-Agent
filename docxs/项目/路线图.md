...
> 2026-08-23 状态复核：M2-1/M2-2/M2-3 的业务功能、权限边界和核心业务测试已完成；M3 的运营审计、DLQ 重放契约、保留治理和受控清理任务代码切片已完成。不可逆数据库硬删除、真实依赖清理、性能/故障矩阵、生产部署和安全演练仍是后置范围。
...
| 范围 | 状态 | 当前边界 |
|---|---|---|
| M2-1 公共知识库与 RAG | deterministic 本地业务闭环已完成，生产强化后置 | 已用 Docker PostgreSQL/Redis/RocketMQ/MinIO 依赖和宿主机 Java/Python 验证批量上传、索引 Outbox、RocketMQ Worker、MinIO 解析、Java 结果回写、发布/下线/恢复、用户检索和 AgentRun `run.completed` 引用；Chat 兼容 SSE 入口已验证 `Last-Event-ID` 回放；Docker 应用镜像、真实 embedding/云模型、性能与故障矩阵后置 |
