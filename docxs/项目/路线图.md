...
> 2026-09-02 状态复核：M2-1/M2-2/M2-3 的业务功能、权限边界和核心业务测试已完成；M2-1 的 Docker deterministic stub/local Milvus 业务闭环已实际验证，local Worker 追加 Milvus readiness 启动门禁。Docker `agent-runtime` 的启动/readiness 和历史凭据下 SiliconFlow Chat、`BAAI/bge-m3`、`Qwen/Qwen3-Embedding-0.6B` 的单次协议证据已记录；D114 当前 Embedding 凭据对两个 profile 均返回 HTTP 401，不能把历史成功证据当作当前调用成功，两个 Embedding profile 仍使用互斥 Milvus collection。M1-4 已增加结构化 Agent 反馈入口、记忆候选三层数据边界和业务测试，M3 的运营审计、DLQ 重放契约、保留治理和受控清理任务代码切片已完成；calculator、plan_validator 的 Java/Python 确定性工具路径及统一工具执行审计已完成并通过定向业务测试。M1-5 营养目录当前含 60 条 approved 食材、60 条 foodPortions 换算和 75 条精确质量换算，V8 seed/validation 已在本地 PostgreSQL 执行通过。不可逆数据库硬删除、真实依赖清理、性能/故障矩阵、生产部署和安全演练仍是后置范围。
...
## M2 功能版当前状态（2026-09-02）

| 范围 | 状态 | 当前边界 |
|---|---|---|
| M2-1 公共知识库与 RAG | deterministic 业务闭环已完成，真实 Embedding 当前凭据待修复，生产强化后置 | 已用 Docker PostgreSQL/Redis/RocketMQ/MinIO/Milvus 以及 `foodmate`/`agent-runtime` 应用容器验证批量上传、索引 Outbox、RocketMQ Worker、MinIO 解析、Java 结果回写、发布/下线/恢复、用户检索和 AgentRun `run.completed` 引用；stub Redis、local deterministic Milvus 已验证，D112 的两个真实 profile 成功证据使用历史凭据，D114 当前凭据均返回 401；性能与故障矩阵后置 |
