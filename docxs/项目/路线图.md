...
> 2026-08-23 状态复核：M2-1/M2-2/M2-3 的业务功能、权限边界和核心业务测试已完成；M2-1 的 Docker deterministic stub/local Milvus 业务闭环也已实际验证。M1-4 已增加结构化 Agent 反馈入口、记忆候选三层数据边界和业务测试，M3 的运营审计、DLQ 重放契约、保留治理和受控清理任务代码切片已完成。不可逆数据库硬删除、真实依赖清理、性能/故障矩阵、生产部署和安全演练仍是后置范围。

## MVP 最短路径（做完这些就能跑）

> **当前进度：M1-2、M1-3 已完成，M1-4 本地 Agent 闭环与结构化反馈业务入口已完成，M1-5 核心业务和写确认跨进程回归已完成本地验收。当前已验证固定 Workflow、受控模型适配、预算、Eval Gate、checkpoint、Proposal/Result、浏览器 SSE、结构化反馈的归属/幂等/审计边界、饮食记录基础读写、5 条营养目录 seed 的匹配、5 条 USDA foodPortions 单位换算规则、营养分析、餐食计划完整资源生命周期、`meal_plan.save_plan` 写确认，以及 `food_log_writer` 在真实 HTTP/RocketMQ 下各 11/11 的 create、拒绝、失败、superseded、update/delete/restore、revision 冲突和幂等重放；默认仍为 `deterministic:local`。更广泛营养目录、生产长压、云模型长稳、价格/账单审计和生产 Eval 治理仍在后续范围。下方 B 编号任务保留为历史拆分清单，状态必须以本文件顶部和 M1-5 实施方案为准。**
...
| 4 | M1-5 | 饮食业务、写确认与最小 Java 工具 | 核心范围已完成，扩展持续 | 饮食记录、营养分析、计划生命周期、完整 `food_log_writer` 写操作、5 条官方单位换算规则和 HTTP/MQ 各 11/11 跨进程回归已完成；更广泛营养目录及生产治理仍后置 |
| 5 | M1-6 | 本地审计、metrics、压测与故障恢复 | 审计/指标代码已完成验证，整体未完成 | 双 JVM/Java 重启已有本机基线；共享依赖业务流量、完整队列与故障矩阵仍待实际证据 |

> **当前已可验证登录 → 对话 → AgentRun → Java Outbox → RocketMQ → Python Redis Inbox → 固定 Workflow/Eval → RocketMQ event → Java PostgreSQL Inbox/SSE，以及独立本地 HTTP 的饮食记录创建/查询/编辑/删除/恢复、5 条目录 matched/pending 营养分析、5 条官方 foodPortions 换算、餐食计划完整资源生命周期和 `meal_plan.save_plan` 写确认；`food_log_writer` 的 11 个 HTTP 和 11 个 RocketMQ 跨进程回归均已通过，包含确认绑定、营养匹配、拒绝/失败/superseded、update/delete/restore、revision 冲突和重放保护。另已完成本机两个独立 Java JVM 共享 PostgreSQL 的有界读取基线和 Java 重启后回读；M1-5 后续主要是更广的官方目录扩展，M1-6 仍未整体完成。**
...
## M2 功能版当前状态（2026-08-23）

| 范围 | 状态 | 当前边界 |
|---|---|---|
| M2-1 公共知识库与 RAG | deterministic 本地业务闭环已完成，生产强化后置 | 已用 Docker PostgreSQL/Redis/RocketMQ/MinIO 以及 `foodmate`/`agent-runtime` 应用容器验证批量上传、索引 Outbox、RocketMQ Worker、MinIO 解析、Java 结果回写、发布/下线/恢复、用户检索和 AgentRun `run.completed` 引用；stub Redis 和 local deterministic Milvus 两种模式均已验证，Chat 兼容 SSE 入口已验证 `Last-Event-ID` 回放；真实 embedding/云模型、性能与故障矩阵后置 |
...
状态：映射结构已补齐（V1 表结构已完成）；用户、资料、会话和消息已完成本地 PostgreSQL 真实读写回归，刷新令牌已接入 Java/API 业务路径并通过轮换、旧 token 拒绝、注销撤销和前端 401 重试测试；头像仍待独立业务写路径验收。
