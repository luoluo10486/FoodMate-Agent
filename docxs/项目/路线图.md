...
> **当前进度：M1-2 与 M1-3 最小真实闭环已完成，M1-4 的 Java/Python RocketMQ + Redis 基础传输闭环已验证，准备进入 M1-4 Agent 能力实现。Python agent-runtime 已完成确定性 stub、Redis Inbox/Outbox、RocketMQ command/event 消费与发布；下一步是受控模型、Router、Planner、预算和 Eval Gate。下方 B 编号任务保留为历史拆分清单，状态必须以本文件顶部和 M1 实施方案为准。**

| 顺序 | 编号 | 任务 | 状态 | 预估 |
|------|------|------|------|------|
| 1 | M1-2 | 真实认证、会话、消息与前端接入 | 已完成 | 已验证 |
| 2 | M1-3 | AgentRun、确定性 Runtime stub 与真实 SSE | 已完成 | 已验证 |
| 3 | M1-4 | RocketMQ 双运行时主通道、LangGraph、模型、预算、Eval 与 checkpoint | 基础闭环已完成，Agent 能力实现中 | RocketMQ/Redis 已验证；其余能力待实现 |
| 4 | M1-5 | 饮食业务、写确认与最小 Java 工具 | 待开始 | 后续 |
| 5 | M1-6 | 审计、可观测性与核心部署 | 待开始 | 后续 |

> **当前已可验证登录 → 对话 → AgentRun → Java Outbox → RocketMQ → Python Redis Inbox → 确定性 stub → RocketMQ event → Java PostgreSQL Inbox/SSE 的真实最小闭环。真实模型、Agent 规划、饮食写入和分析仍未完成。**
...
M1-4 的唯一运行架构入口为[Agent 运行架构](../架构/Agent运行架构.md)，RocketMQ 决策见[ADR-0005](../决策/ADR-0005-RocketMQ异步主通道.md)，Runtime 工程边界见[Python 智能体运行时设计](../架构/Python智能体运行时设计.md)，全部默认值和环境变量见[配置指南](./配置指南.md)。当前确认的是目标架构，不代表代码、Compose、Flyway 或 wire transport binding 已支持。
