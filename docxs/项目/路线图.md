...
| 3 | M1-4 | Python Runtime：LangGraph 编排、模型、预算、Eval、checkpoint 与生产治理 | 当前任务 | 架构已确认，待实施方案与代码 |
...
M1-4 的唯一架构入口为[Agent 运行架构](../架构/Agent运行架构.md)，Runtime 工程边界见[Python 智能体运行时设计](../架构/Python智能体运行时设计.md)，全部默认值和环境变量见[配置指南](./配置指南.md)。当前确认的是目标架构，不代表代码、Flyway 或 V1 wire 已支持。
