...
> **当前进度：M1-2 与 M1-3 最小真实闭环已完成，当前进入 M1-4。Python agent-runtime 已创建并完成确定性 stub、Service JWT、dispatch/cancel 与事件回调；下一步是受控模型、Router、Planner 与执行引擎。下方 B 编号任务保留为历史拆分清单，状态必须以本文件顶部和 M1 实施方案为准。**

| 顺序 | 编号 | 任务 | 状态 | 预估 |
|------|------|------|------|------|
| 1 | M1-2 | 真实认证、会话、消息与前端接入 | 已完成 | 已验证 |
| 2 | M1-3 | AgentRun、确定性 Runtime stub 与真实 SSE | 已完成 | 已验证 |
| 3 | M1-4 | RocketMQ 双运行时主通道、LangGraph、模型、预算、Eval 与 checkpoint | 当前任务 | 架构已确认，待代码与本地验证 |
| 4 | M1-5 | 饮食业务、写确认与最小 Java 工具 | 待开始 | 后续 |
| 5 | M1-6 | 审计、可观测性与核心部署 | 待开始 | 后续 |

> **当前已可验证登录 → 对话 → AgentRun → 确定性 stub → SSE 的真实最小闭环。真实模型、Agent 规划、饮食写入和分析仍未完成。**
