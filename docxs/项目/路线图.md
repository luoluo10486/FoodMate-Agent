...
> **当前进度：M1-2、M1-3 已完成，M1-4 本地 Agent 闭环已完成。当前已验证固定 Workflow、受控模型适配、预算、Eval Gate、checkpoint、Proposal/Result、浏览器 SSE 和恢复入口；默认仍为 `deterministic:local`。生产长压、云模型长稳、价格/账单审计、生产 Eval 治理、RAG 和完整业务 Tool/SQL 仍在后续范围。下方 B 编号任务保留为历史拆分清单，状态必须以本文件顶部和 M1-4 实施方案为准。**
...
| 4 | M1-5 | 饮食业务、写确认与最小 Java 工具 | 决策已完成，代码待实现 | 先手工记录，再分析、计划和确认 |
| 5 | M1-6 | 本地审计、metrics、压测与故障恢复 | 范围已收窄，代码待实现 | 生产监控、部署、备份和回滚后置 |

> **当前已可验证登录 → 对话 → AgentRun → Java Outbox → RocketMQ → Python Redis Inbox → 固定 Workflow/Eval → RocketMQ event → Java PostgreSQL Inbox/SSE 的本地真实闭环。M1-5 已完成决策但尚未改造代码；真实饮食写入、营养分析和餐食计划仍未完成。**
