...
> 2026-08-27 状态复核：M2-1/M2-2/M2-3 的业务功能、权限边界和核心业务测试已完成；M2-1 的 Docker deterministic stub/local Milvus 业务闭环也已实际验证。M1-4 已增加结构化 Agent 反馈入口、记忆候选三层数据边界和业务测试，M3 的运营审计、DLQ 重放契约、保留治理和受控清理任务代码切片已完成；calculator、plan_validator 的 Java/Python 确定性工具路径及统一工具执行审计已完成并通过定向业务测试。不可逆数据库硬删除、真实依赖清理、性能/故障矩阵、生产部署和安全演练仍是后置范围。
