...
> 传输更新（2026-07-26）：本文中的历史 HTTP/Runtime Client 示例只作为概念或 M1-3 兼容参考。Java/Python 目标正式异步主通道已确定为 RocketMQ 普通消息；PostgreSQL Outbox/Inbox、Redis 调度与 Python 技术状态、Topic 和 DLQ 规则以[ADR-0005](../决策/ADR-0005-RocketMQ异步主通道.md)和[Agent 运行架构](./Agent运行架构.md)为准。
...
- 本节是模型网关/模型平台的历史示例，字段命名以[数据库设计](../数据/数据库设计.md)为准；业务表主键不要使用泛化 `id`
...
#### 8.1.8 UserFoodLog（M1-5 目标结构）

饮食记录不再使用单个 JSON 字段承载食材和营养。`food_logs` 保存餐次主表字段；`food_log_items` 保存原始食材、份量、营养目录匹配、单位换算和营养快照；`nutrition_foods` 保存目录基准值。具体字段以 [数据库设计](../数据/数据库设计.md) 和 [M1-5 实施方案](../项目/M1-5核心饮食业务与写确认实施方案.md) 为准。
...
| `query-understanding/default_query_understanding.md` | 查询理解 | 改写后的查询、实体、过滤条件、澄清需求 |
| `sql-agent/default_sql_agent.md` | 只读 SQL 提案 | 数据源、SQL 草案、限制条件、过滤意图 |
| `validator/default_validator.md` | 结果校验 | 是否通过、问题列表、修正建议 |
...
1. [产品需求文档](../产品/产品需求文档.md)
2. [系统设计与技术方案](./系统设计与技术方案.md)
3. [智能体行为与工具协议](../契约/智能体行为与工具协议.md)
4. [接口与数据规范](../契约/接口与数据规范.md)
5. [Java 控制面工程设计](./Java控制面工程设计.md)
6. [数据库设计](../数据/数据库设计.md)
7. [实现附录](../参考/实现附录.md)
