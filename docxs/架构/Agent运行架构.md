...
> D182 当前业务门禁：Java application/infra/api 目标 reactor 测试 `12/257/119/72` 全部通过（Infrastructure 条件跳过 `20`）；Python `247 passed、2 skipped、9 subtests`；前端 `46` 个测试文件、`306 passed`，typecheck/build 通过。该证据只覆盖业务正确性，不覆盖生产容量、长稳或完整故障恢复。
...
- 当前已实现本地 deterministic Composer、独立 Eval、预算动作、Redis checkpoint、RocketMQ Event/Proposal/Result、Tool Result 回注、Java 恢复入口、浏览器 SSE 和公共知识库 RAG；M2-1 的批次索引/可见性、M2-2 的 Tool/SQL 业务闭环和 M2-3 的管理核心切片均已有业务证据。D174 已取得正式 WHO 资料的真实 Embedding/Milvus/Chat 引用闭环，R2/R3/R4 也已有单次真实云业务闭环；真实云 RAG/模型长稳、生产级长压、多实例业务流量和完整故障矩阵仍未完成。
...
- Docker Runtime 的启动和 readiness 已验证；D174 已取得当前配置下正式资料的真实 Embedding + Milvus + Chat 单次业务闭环，D114 的 401 属于历史凭据记录。该证据不代表云模型长稳、正式价格对账或生产容量。
...
当前已实现最近 8 条消息、结构化摘要（goals/constraints/decisions/open_questions/source_message_ids）、摘要版本/CAS、候选写入、Java 白名单/来源/冲突校验和用户管理接口；计划型记忆自动分配 7 天 TTL，临时型记忆自动分配 24 小时 TTL，过期记录不会参与冲突判断或 Context 注入。消息更正/删除后的最小摘要失效与重建、来源抑制和按 AgentRun 意图的类型过滤已接入；D180 又补充了偏好、预算范围、烹饪能力、回答语言自然表达及一次性要求过滤。语义衰减、删除防再生的完整缓存传播和向量记忆仍未实现。配置默认值以[配置指南](../项目/配置指南.md#56-上下文会话摘要与记忆)为准。
