...
Prompt 必须分开标记权威业务事实、已确认长期记忆、Session 摘要和未确认候选。记忆文本只能作为数据，不能成为指令；任何嵌入记忆文本中的操作指令均须忽略。当前“按更新时间取最近 20 条”的实现只是最小闭环，必须迁移为按意图检索。
...
当前已实现最近 8 条消息、确定性摘要、摘要版本/CAS、候选写入、Java 基础校验和用户管理接口。尚未实现结构化语义摘要、按意图检索、TTL 自动分配、衰减、删除防再生以及记忆变更后的完整派生失效。配置默认值以[配置指南](../项目/配置指南.md#56-上下文会话摘要与记忆)为准，不能把本节目标设计误写成已经落地。
