...
- M1-5 核心业务已进入代码并通过本地验证：手工饮食记录创建/查询/编辑/删除/恢复、首批目录 matched/pending 营养分析、5 条 USDA foodPortions 换算、餐食计划完整资源生命周期和 `meal_plan.save_plan` 写确认统一由 Java application 用例编排；`food_log_writer` 的 create/update/delete/restore 已完成 Proposal/Confirm/Execute，并通过真实 PostgreSQL HTTP/RocketMQ 各 11/11 跨进程回归，覆盖 rejected/failed/superseded、revision 冲突和幂等重放。计划生命周期已完成 PostgreSQL HTTP 回归，营养目录和确认事实落 PostgreSQL，当前有 5 条 approved 食材 seed 和 5 条 approved 换算规则；M2-1 公共知识库、M2-2 Tool/SQL 和 M2-3 管理核心切片已分别完成当前业务门禁。详细表结构以 [M1-5 实施方案](../项目/M1-5核心饮食业务与写确认实施方案.md) 为准。
