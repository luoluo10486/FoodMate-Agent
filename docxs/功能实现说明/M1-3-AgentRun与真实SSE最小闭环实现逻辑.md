...
> 后续演进说明（2026-07-28）：本文只记录已经验证的 M1-3 HTTP Runtime stub 实现。M1-4 已在其上完成 RocketMQ/Redis 基础传输闭环，但 LangGraph、真实模型、Eval 和完整记忆治理仍未实现；不得把后续基础设施或目标能力倒写成 M1-3 当时的实现事实。
