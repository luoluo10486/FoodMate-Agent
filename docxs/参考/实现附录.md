...
版本：v2.1
维护基线：2026-07-11
文档定位：这是 FoodMate 项目的实施细节附录，负责补充执行流程、字段样例、模板、落地建议和验收细则。架构、职责边界、查询主路径、工具优先级以总设计文档为准；本文件不再作为唯一总规格来源。

统一架构前提：Java 是业务控制面和数据权威，Python 是 Agent 智能执行面。本文中的 Router、Planner、RAG、ModelService、Prompt、Composer 和评测默认属于 Python；工具策略、业务写入、SQL Guard、只读执行、AgentRun 和审计默认属于 Java。

对应文档：

- [FoodMate-系统设计与技术方案.md](./FoodMate-系统设计与技术方案.md)
- [FoodMate-产品需求文档.md](./FoodMate-产品需求文档.md)
- [FoodMate-接口与数据规范.md](./FoodMate-接口与数据规范.md)
- [FoodMate-智能体行为与工具协议.md](./FoodMate-智能体行为与工具协议.md)
...
这份文档就是为这些问题准备的。总设计负责下最终结论，本附录负责把这些结论展开成可实施的细节和样例。
...
FoodMate 是一个面向餐饮与饮食管理场景的任务型 Agent 系统，支持自然语言交互、知识检索、工具调用、饮食记录、数据分析和多步规划。
...
1. 用户输入自然语言，Java 完成鉴权
2. Java 保存用户消息并创建 AgentRun
3. Java 将版本化 RunCommand 派发给 Python
4. Python Router 识别意图并判断是否缺参
5. Python 追问或生成任务计划
6. Python 执行模型/RAG，或向 Java 提交工具、SQL 和候选记忆 proposal
7. Java 完成授权、确认、Guard、事务执行和审计
8. Python 校验质量并生成最终结构化输出
9. Python 回传有序 RunEvent
10. Java 去重、校验状态、保存轨迹并输出前端 SSE
...
```json
{
  "profile_id": "1912345678901234001",
  "user_id": "1912345678901234001",
  "name": "wyh",
  "diet_preferences": {
    "avoid": ["pork"],
...
| 状态 | 描述 | 进入条件 | 退出条件 |
|---|---|---|---|
| active | 正常使用中 | 新建会话 | 归档/关闭 |
| archived | 已归档 | 用户归档 | 恢复 |
| closed | 已关闭 | 用户主动结束或系统关闭 | 恢复为 active 或归档 |
...
- 状态变化必须能回放
- 状态变化必须可从日志追踪
- `deleted` 不再作为业务状态值，删除语义统一通过 `is_deleted`、`deleted_at`、`deleted_by` 表达

### 9.5 Mapper / Service 与软删除约束

- 所有 MyBatis-Plus Mapper / Service 默认查询必须附带 `is_deleted = false`
- 只有显式的管理接口查询才允许绕过软删除过滤
- 业务代码禁止手写“全量查”跳过软删除
- 删除操作必须写入 `deleted_at`、`deleted_by` 和对应审计日志
- 恢复操作必须记录恢复审计，并恢复默认可见性
- 唯一约束默认仅对 `is_deleted = false` 的数据生效
...
RAG 不建议直接拿原始问题去召回，建议先做查询理解。问题改写只是其中一步，这一层还要负责把问题整理成可召回、可过滤、可审计的结构化输入。
...
FoodMate 建议把 NER、实体归一化、时间归一化、ACL 条件构建都放进查询理解层。  
因为很多时候，单纯生成 query 还不够，最好先识别实体和约束，再决定如何召回。
...
FoodMate 建议把 ACL 过滤作为检索链路中的硬约束层，而不是最后才补一个权限判断。
...
说明：这些 ACL 字段属于未来 ABAC/多租户知识库扩展的推荐元数据；MVP 鉴权角色仍固定为 `user/admin/operator`，部门级可见性只作为后续扩展口径。
...
FoodMate 知识库不是简单的向量入库，而是一个完整的数据管道。
...
FoodMate 推荐采用 **Plan-Act-Observe-Reflect** 范式，并结合有限状态机执行。
...
| 查询工具 | database_query | 读操作为主，主路为 SQL Agent + MCP |
...
FoodMate 的工具不需要一开始就做很多，优先把“最小可用工具集”做稳。
...
| `meal_plan_generator` | 多天备餐计划生成 | 规划模板和校验流程稳定后 |
...
下面两项保留为内部能力模块，不作为对外工具注册：

| 模块 | 作用 |
|---|---|
| `entity_normalizer` | 食材、菜品、人名、公司名标准化 |
| `prompt_router` | 任务路由与 Prompt 选择 |
...
如果用户问的是“知识库里没有，但希望得到更详细的公开信息”，推荐按下面的顺序处理：
...
- 能用结构化外部来源就不要直接抓网页
- Browser Use 只作为兜底工具
- 结构化数据查询主路默认走 `database_query -> SQL Agent -> MCP`
...
### 16.4 Java + Python 实现建议

系统固定为 Java 业务控制面 + Python Agent Runtime：

#### 服务划分

- Java `api`：对外 Controller、鉴权入口、AgentRun 和 SSE 输出
- Java `agentclient`：Run 派发、事件接收、服务身份、超时和取消
- Python `agent-runtime`：Agent 编排、Prompt、模型、RAG、SQL proposal 和评测
- Java `tool/sqlaccess`：策略、确认、业务事务、SQL Guard、执行和审计
- Java/Python `worker`：业务异步任务与索引/评测任务分别归属
- Java `persistence`：业务数据库访问，封装 MyBatis-Plus Mapper / Service
...
- `Python 3.12+`
- `FastAPI + Pydantic v2`
- `LangGraph` 优先评估，轻量场景可选择 OpenAI Agents SDK 或自研状态机
- `pytest`
...
- Controller 不写业务逻辑
- Python Orchestrator 只做推理和编排，不直接操作业务库
- Java Tool/SQL 只负责受控执行
- Python Retriever 只在授权范围内召回和整理证据
- Python Validator 负责质量；Java Policy/Validator 负责权限、确认和业务规则
- Java 与 Python 只共享版本化语言无关契约
...
FoodMate 里最值得单独强调的一项能力就是 SQL Agent。  
...
这一节对应 Python Runtime 内的 `ModelService`。它解决的问题是：**Agent 组件不要直接散落供应商 API 调用，而是统一通过 Python 模型适配层访问模型能力。** Java 业务模块不调用模型，也不参与 Prompt 装配。
...
#### 20.2.2 `ModelGatewayClient`

负责把业务请求发往模型网关，并承接统一请求头、trace id、超时和响应反序列化。  
如果第一阶段还没有独立模型网关，可以先以内嵌模块实现同样接口。
...
3. `ModelGatewayClient` 将请求发送到模型网关
4. 模型网关根据策略选择具体供应商
5. `RetryPolicy` 和超时策略生效
6. `ProviderAdapter` 发起请求
7. `ModelCallLogger` 记录日志
8. `CostMeter` 统计成本
9. 结果返回 Python Agent 组件；用量事件回传 Java 持久化
...
FoodMate 的 Python Runtime 中建议统一走 `ModelService` 的地方包括：
...
- Java 业务代码不依赖供应商 SDK，Python Agent 组件只依赖统一 ModelService
- 所有请求都能记录日志
- 所有请求都能统计成本
- 支持流式和非流式输出
- 支持结构化输出
...
1. Python Agent Runtime 调用 `ModelService`
...
- `ModelService` 面向 Python Agent Runtime，负责统一调用入口
- 模型网关面向治理，负责统一控制和分发
- `ProviderAdapter` 面向供应商，负责协议适配

如果系统还小，可以先把模型治理逻辑放在 Python `ModelService` 内部。
如果后续供应商很多、租户很多、策略很多，再把它拆成独立服务。

### 21.7 FoodMate 的落地建议

FoodMate 里模型网关最应该接管的内容：
...
用户给出的这张图可以直接作为 FoodMate 的模型网关参考架构。  
...
- Python `agent-runtime` 只依赖 `ModelService`
- `ModelService` 再进入模型网关
- 模型网关统一做权限、路由、限流、日志、成本和重试
- 网关根据策略把请求转发给不同模型供应商
- Java 业务模块不接触供应商 SDK；Python Agent 组件也不直接管理 Key

### 21.11 推荐实现原则

- 先作为 Python Runtime 内部模块，不额外增加第三个部署单元
...
### 21.13 模型网关数据库设计示例（历史参考，不实施）

下面这组表是根据你给的“用户表 / 模型配置表 / 调用日志表”参考图整理出来的。  
以下 SQL 不属于当前实施方案，不得加入 Flyway。Python Runtime 不拥有用户、权限、路由规则、模型用量或审计数据库；它只执行模型调用并回传版本化用量事件。Java 控制面拥有 `users`、`model_usage_logs`、`model_route_rules` 等权威数据并负责幂等持久化，供应商凭据由部署 Secret/Vault 管理。

#### 21.13.1 `users`（禁止在 Python 侧重复创建）
...
CREATE TABLE model_providers (
    model_provider_id BIGINT PRIMARY KEY,
