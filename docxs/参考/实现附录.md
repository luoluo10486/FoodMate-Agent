...
> **非权威参考：** 本附录用于补充实现细节，不替代代码、迁移、测试、ADR 或契约。历史模型网关 SQL 与旧 Java Agent 示例仅供追溯，**不进入当前实施**。
...
│   └── default_planner.md
├── query-understanding/
│   └── default_query_understanding.md
├── sql-agent/
│   └── default_sql_agent.md
├── validator/
│   └── default_validator.md
├── composer/
│   └── default_composer.md
```
...
| `query-understanding/default_query_understanding.md` | 查询改写、实体与过滤条件提取 |
| `sql-agent/default_sql_agent.md` | 只读 SQL proposal 生成 |
| `validator/default_validator.md` | 工具与结果质量校验 |
...
### 21.13 模型网关数据库设计示例（历史参考，不进入当前实施）
