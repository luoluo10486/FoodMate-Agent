...
- [x] 已完成 Figma 节点 `640:588`（Diet Records）的一次增量浏览器证据登记：`1440×1024`、DPR `1`、字体加载完成、无页面溢出，自动 diff 为 `7.372979% / MAE 2.469032 / RMSE 16.609406 / maxChannelDelta 255`。
- [x] 该节点在 `figma-105-mapping.json` 中保持 `DIFF_REVIEW`；本次没有重新验收其余画板，也没有将非零差异标记为 `PASS`。
- [x] 默认人物头像的运行时资源和实时 Figma Design 页已分别核对：本地仅登记用户提供的男性/女性 SVG；Figma 192 个默认头像容器中男性 189、女性 3、其它来源 0。认证页 8 个 18×18 `user` 节点属于字段装饰图标，不属于人物头像。
- [ ] iconfont 实体包、CSS/Unicode 映射、来源、许可证和 glyph-Figma 映射仍未提供，继续保持 `BLOCKED`。
- [x] 前端头像组件默认改为 `defaultOnly=true`；真实上传预览保留双重显式授权，新增调用方不会默认渲染外部或历史人物图片。
