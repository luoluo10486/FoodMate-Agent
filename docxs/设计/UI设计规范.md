...
状态：Figma 重构前置规范
日期：2026-08-05
详细来源：[产品需求文档](../产品/产品需求文档.md)、[实现附录](../参考/实现附录.md)、[Figma 前端重构设计需求规格](./Figma前端重构设计需求规格.md)
...
- 信息密度适中，但必须易读。
- 以 Figma 高保真设计稿作为布局、视觉、组件变体和状态展示的设计源；以本文件和接口契约作为行为与内容边界。
- 目标重构使用 shadcn/ui 的可组合组件和语义化 CSS Token，不再为新页面引入 Arco Design。
- 标准控件图标优先使用与 shadcn/ui 生态一致的图标实现；Figma 或品牌资产中存在准确的领域矢量图时，使用 iconfont 资源并通过语义化名称映射。
...
- shadcn/ui 只提供可访问的控件基础，不提供页面模板；工作台布局、信息层级、食品领域视觉和状态表达必须由 FoodMate 自定义。
...
![会话页原型](./资源/原型图/会话页原型.svg)

### 3.3 分析页原型

![分析页原型](./资源/原型图/分析页原型.svg)

### 3.4 规划页原型

![规划页原型](./资源/原型图/规划页原型.svg)

### 3.5 个人资料页原型

![个人资料页原型](./资源/原型图/个人资料页原型.svg)

### 3.6 管理后台原型

![管理后台原型](./资源/原型图/管理后台原型.svg)

### 3.7 管理后台子页面原型

![管理后台用户管理原型](./资源/原型图/管理后台-用户管理原型.svg)

![管理后台 Agent 运行原型](./资源/原型图/管理后台-Agent运行原型.svg)

![管理后台工具调用原型](./资源/原型图/管理后台-工具调用原型.svg)

![管理后台模型用量原型](./资源/原型图/管理后台-模型用量原型.svg)

![管理后台知识库原型](./资源/原型图/管理后台-知识库原型.svg)

![管理后台软删除资源原型](./资源/原型图/管理后台-软删除资源原型.svg)

## 3.8 shadcn/ui 视觉策略

现有 12 张 SVG 原型图是信息结构基准，Figma 高保真稿是后续实现的视觉基准。实现时使用 shadcn/ui 的基础组件、Radix 可访问行为和 FoodMate 自定义 Token，但不能改变以下核心结构：
...
- 管理后台必须使用独立后台布局，保留角色权限提示、分页筛选、审计列表和高风险操作确认入口。
- 管理后台左侧导航必须能跳转 `/admin/users`、`/admin/runs`、`/admin/tools`、`/admin/usage`、`/admin/knowledge`、`/admin/deleted` 等子页面。

shadcn/ui 组件映射：

| shadcn/ui 组件 | FoodMate 使用方式 |
|---|---|
| `Button` | 发送、停止、确认、取消、重试、快捷任务操作 |
| `Input` / `Textarea` | Composer、搜索会话、追问回复 |
...
- Figma 负责页面结构、组件变体、间距、颜色、文字样式和关键状态；接口契约负责字段、事件、权限和错误码。
- Figma Variables、颜色样式和文字样式必须能够映射到 `foodmate-ui/src/styles/tokens.css` 的语义化 CSS 变量，不在业务组件中散落硬编码颜色。
- Figma 组件名、Variant 属性名和代码组件名保持稳定；页面只组合共享组件，不复制一份页面专用控件。
- 设计稿中的标准控件优先映射到 `components/ui`，FoodMate 领域组件映射到 `components` 或对应 `features`，两者不混用职责。

图标与矢量资产约定：

- 熟悉的发送、停止、搜索、编辑、删除、复制、下载、刷新和展开命令使用统一的代码图标组件，并保证 `aria-label` 和 Tooltip 完整。
- Figma 提供的品牌、食物、营养或状态领域图标，在 iconfont 有准确资源时使用 iconfont；代码通过稳定的语义 class 或 React 封装组件调用，不直接写 Unicode 字符或随机内联 SVG。
- iconfont 没有对应资源时，才使用许可明确的 Figma SVG 资产或代码图标；新增资源必须登记名称、来源、尺寸和明暗背景适配情况。
...
- 用户管理：用户列表、用户详情、登录会话、启用、禁用、锁定、重置登录会话。
- 运行审计：AgentRun、ToolCall、SQLAudit、Trace 查询。
- 模型用量：ModelUsage 独立页面，展示供应商、模型、场景、token、成本和耗时。
...
- 普通用户不能看到管理入口。
- `operator` 可进入只读治理视图；用户管理、软删除资源等 admin-only 页面展示权限不足状态。
- `operator` 看到的高风险按钮必须隐藏或禁用。
...
- 当前阶段只改造设计与实现文档，不修改前端代码、依赖和现有 Figma 文件；现有代码中的 Arco 仅代表迁移前基线。
- 第一轮前端实现优先做首页和会话页。
- 分析页和规划页第一阶段可以使用静态数据。
- 工具失败不能被隐藏，需要以用户可理解的方式展示。
- 不能展示原始内部 Prompt 或推理过程。
- API 事件名和运行状态应遵循[接口与数据规范](../契约/接口与数据规范.md)。

## 11. 文档定位

本文是 UI 和交互入口。更完整的页面样例和实现细节仍以[实现附录](../参考/实现附录.md)为准。
