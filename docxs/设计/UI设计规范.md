...
![管理后台用户管理原型](./assets/prototypes/管理后台-用户管理原型.svg)

![管理后台 Agent 运行原型](./assets/prototypes/管理后台-Agent运行原型.svg)

![管理后台工具调用原型](./assets/prototypes/管理后台-工具调用原型.svg)

![管理后台模型用量原型](./assets/prototypes/管理后台-模型用量原型.svg)

![管理后台知识库原型](./assets/prototypes/管理后台-知识库原型.svg)

![管理后台软删除资源原型](./assets/prototypes/管理后台-软删除资源原型.svg)

## 3.8 Arco Design 视觉策略

现有 12 张 SVG 原型图是信息结构基准，不是逐像素高保真稿。实现时可以用 Arco Design 优化控件质感、间距、状态反馈和交互细节，但不能改变以下核心结构：
