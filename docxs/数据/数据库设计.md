...
实现基线：2026-07-10；M1-5 当前复核（2026-08-14）：`food_logs` 已调整为主表 + 明细表，V13/V14/V15 结构已在本地 `FoodMate` 库中复核，V15 已人工执行并通过校验。当前数据量为 `food_logs=8`、`food_log_items=11`、`nutrition_foods=5`、`nutrition_unit_conversions=0`、`approval_requests=6`；营养目录 V1 seed 已人工导入并通过校验，单位换算暂不推断。`food_log_writer` 的 `food_log.create` 已完成本地第一切片和真实 HTTP/MQ 回归。数据库人工脚本为 [`V1__init_core_schema.sql`](../../script/sql/FoodMate/baseline/V1__init_core_schema.sql)、后续版本化增量 SQL 和 [`seed/V1__nutrition_usda_seed.sql`](../../script/sql/FoodMate/seed/V1__nutrition_usda_seed.sql)。
...
> M1-5 当前实现边界（2026-08-14）：`food_logs`、`food_log_items`、`nutrition_foods`、`nutrition_unit_conversions` 和 `approval_requests` 已存在于本地数据库；Java 已实现饮食记录第一切片、分析、餐食计划完整资源生命周期、`meal_plan.save_plan` 确认执行和 `food_log_writer` `food_log.create` 本地第一切片，并通过真实 PostgreSQL HTTP/RocketMQ 回归。`nutrition_foods` 当前有 5 条已审核 seed，`nutrition_unit_conversions` 为 0；不能把没有换算规则的家庭单位当作可安全计算。营养数值来自数据库目录，`NUMERIC` 字段由 Java `BigDecimal` 读写和计算。
...
约束：`food_log_id + item_order` 唯一；`amount > 0`；`nutrition_status = matched` 时必须有目录匹配、标准化份量和营养快照。`pending` 明细允许保存但不计入汇总。
...
以下接口是前端和 Worker 的目标契约，当前 Java 实现尚未完成：

| 方法 | 路径 | 用途 | 鉴权 | 返回 |
|---|---|---|---|---|
| `POST` | `/foodmate/admin/knowledge-document-batches` | 批量上传文件并创建索引批次 | `admin` | `202`，返回 `batch_id` 和文件级初始状态 |
| `GET` | `/foodmate/admin/knowledge-document-batches/{batch_id}` | 查询批次汇总、文件进度和错误 | `admin/operator` | `200`，返回批次状态和文件列表 |
| `POST` | `/foodmate/admin/knowledge-document-batches/{batch_id}/retry-failed` | 仅重试失败文件 | `admin` | `202`，返回新的任务版本和待处理数量 |
...
批次状态：`queued`、`running`、`completed`、`partial_failed`、`failed`、`expired`。
...
目标持久化模型为 `knowledge_import_jobs`（批次）和 `knowledge_import_items`（文件项），并通过 `document_id` 关联现有 `knowledge_documents`。批次创建、文件元数据写入和 Outbox 事件必须在同一事务中完成；Worker 消费必须使用 `job_id`、`item_id` 和幂等键吸收重复消息。当前数据库尚未落地这两张任务表，不能把本节契约标记为后端已实现。
