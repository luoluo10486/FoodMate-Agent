...
> M1-5 当前实现边界（2026-08-15）：`food_logs`、`food_log_items`、`nutrition_foods`、`nutrition_unit_conversions` 和 `approval_requests` 已存在于本地数据库；Java 已实现饮食记录、分析、餐食计划完整资源生命周期、`meal_plan.save_plan` 确认执行和 `food_log_writer` create/update/delete/restore，并通过真实 PostgreSQL HTTP/RocketMQ 各 11/11 回归验证 rejected/failed/superseded、revision 冲突和幂等重放。`nutrition_foods` 当前有 5 条已审核 seed，`nutrition_unit_conversions` 当前有 5 条已审核 USDA foodPortions 规则；没有官方换算证据的家庭单位仍不计算。营养数值来自数据库目录，`NUMERIC` 字段由 Java `BigDecimal` 读写和计算。
