...
更新摘要时必须以 `session_id + summary_version` 做 CAS；消息删除或更正导致 `source_digest` 失效时，不允许继续装配旧摘要，必须从仍有效消息重建。长期记忆继续由 `user_memories` 保存，Python 只提交候选，Java 校验并负责查看、更正、删除与审计。
