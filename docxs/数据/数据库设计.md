...
实现基线：2026-07-10；M1-5 当前复核：`food_logs` 已调整为主表 + 明细表，V13/V14/V15 结构已在本地 `FoodMate` 库中复核，V15 已人工执行并通过校验。当前数据量为 `food_logs=1`、`food_log_items=1`、`nutrition_foods=0`、`nutrition_unit_conversions=0`、`approval_requests=1`；营养目录表已建但尚未导入官方 seed。`food_log_writer` 的 `food_log.create` 已完成本地第一切片，但真实 HTTP/MQ 回归尚未完成。数据库人工脚本为 [`V1__init_core_schema.sql`](../../script/sql/FoodMate/baseline/V1__init_core_schema.sql) 和后续版本化增量 SQL。
