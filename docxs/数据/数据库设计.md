...
| `status` | `VARCHAR(32)` |  | 当前 V1：`queued/routed/waiting_user/planning/retrieving/executing/validating/completed/failed/cancelled`；目标增加 `superseded` |
...
| `failed` | 失败 |
| `cancelled` | 已取消 |
| `superseded` | 目标终态：旧 Run 已由同 Session 的 continuation Run 接续；当前 V1 尚未迁移 |
