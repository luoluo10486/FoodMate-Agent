...
实现基线：2026-07-10；M1-5 当前复核：`food_logs` 已调整为主表 + 明细表，V13/V14 结构已在本地 `FoodMate` 库中只读复核。当前数据量为 `food_logs=1`、`food_log_items=1`、`nutrition_foods=0`、`nutrition_unit_conversions=0`、`approval_requests=1`；营养目录表已建但尚未导入官方 seed。数据库人工脚本为 [`V1__init_core_schema.sql`](../../script/sql/FoodMate/baseline/V1__init_core_schema.sql) 和后续版本化增量 SQL。
...
> M1-5 当前实现边界（2026-08-13）：`food_logs`、`food_log_items`、`nutrition_foods`、`nutrition_unit_conversions` 和 `approval_requests` 已存在于本地数据库；Java 已实现饮食记录第一切片、分析、计划基础流程和 `meal_plan.save_plan` 确认执行。营养目录目前为空，不能把表存在误写成已经有热量/蛋白质/脂肪/碳水数据。`NUMERIC` 字段由 Java `BigDecimal` 读写和计算。
