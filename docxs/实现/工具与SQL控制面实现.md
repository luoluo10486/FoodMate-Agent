...
> M1-5 更新（2026-08-15）：`meal_plan.save_plan` 的 Proposal -> Confirm -> Execute 已实现；`food_log_writer` 的 create/update/delete/restore 也已完成，确认事实使用 `approval_requests`，CAS/幂等执行和 `operation_audits` 审计已验证，并通过真实 PostgreSQL HTTP/RocketMQ 跨进程回归。拒绝/失败/superseded 和重放语义已有 HTTP/MQ 证据；更广泛工具调用仍待实现。本文仍是控制面蓝图，不代表完整业务工具链已完成。
...
- 当前 V1 `tool_registries` 未完整包含这些列。实施时只能使用已版本化的 `tool_schema_versions.schema_json` 或 V2 阶段 1 的追加列补齐，不能声称 V1 已有字段，也不能回改 V1。
...
V2 阶段 1 正式定义 `admission_epoch/cancellation_epoch`、dispatch fence、执行租约和 owner token；阶段 4 启用 Tool/SQL 主链路前必须完成表列、CHECK、索引和 PostgreSQL Testcontainers 竞态 fixture。Tool 和 SQL 创建必须与取消/admission 竞争同一 `agent_runs` 行锁；不能一条路径锁行、另一条路径无条件更新。

推荐目标字段：

| 目标对象 | 目标字段/状态 | 约束 |
|---|---|---|
| `agent_runs` | `admission_state/admission_epoch` | admission 关闭、重开或运维 fence 时递增；closed 禁止新 invocation |
| `agent_runs` | `cancellation_epoch` | 首次接受新取消意图时在行锁内递增；相同 cancel 重放不递增 |
| `tool_calls`、`sql_query_audits` | `admission_epoch/cancellation_epoch` | 创建时复制 Run 当前 epoch |
| `tool_calls`、`sql_query_audits` | `dispatch_fencing_token` | 创建时复制当前 active dispatch fence |
...
```text
cancel transaction: lock agent_runs row -> verify cancel idempotency -> increment cancellation_epoch -> close invocation admission -> commit
invocation transaction: lock same agent_runs row -> require admission open/current active dispatch -> copy both epochs and fence -> insert pending placeholder -> commit
```

若 invocation 先拿到锁，取消或 admission fence 随后提交会递增对应 epoch；invocation 在进入 Adapter 前、提交业务副作用前和持久化结果前必须重新锁定/比较两个 epoch、`dispatch_fencing_token`、active dispatch 与 Run 状态，任一不匹配即返回 cancelled/`RUN_CANCELLED` 或稳定 state conflict，不产生新副作用。若取消/admission close 先提交，后续 proposal 直接拒绝。不能仅在 proposal 入站时检查一次。

一个 invocation 同时只能有一个 executor。创建 `pending` 占位后，executor 使用 `UPDATE ... WHERE execution_state='pending'` 的 CAS 写入随机 `owner_token` 和 `lease_until`，领取成功者才可执行。相同 digest 并发命中 `pending` 或 `leased` 时不再执行第二次，而是在本次请求 `deadline_at` 内等待原 owner 完成；变为 `succeeded/failed` 后立即返回已持久化的 ToolResult/SqlResult，并仅把不参与 result digest 的 `meta.replayed` 改为 true。

若到本次请求 deadline 前结果仍不可用，返回既有 RuntimeError：HTTP 503、`error.code=RUNTIME_UNAVAILABLE`、`error.retryable=true`。调用方重试必须保持相同 `invocation_id/idempotency_key/request_hash` 和完整摘要字段，只允许传输 `request_id/trace_id` 按契约变化。不得返回异步占位响应、重试提示 Header、轮询 token、临时结果类型或任何第九类消息。

lease 过期只允许对声明 `retryable=true` 且 `idempotent=true` 的安全工具接管；CAS 必须限定旧 owner/lease 已过期，签发新 owner token、递增 lease attempt，并再次通过 admission/cancellation epoch、dispatch fence、deadline 和 policy 校验。外部副作用在 owner 崩溃后无法确认时必须进入内部 `unknown`，禁止自动重放；Adapter 必须使用 provider idempotency key，或转人工/运维对账。对外不得返回 `unknown` 新状态：已确认 invocation 语义时映射为 ToolResult `failed`/`TOOL_FAILED` 或 SqlResult `failed`/`SQL_EXECUTION_FAILED`；只能表达运行内协议故障时返回稳定 RuntimeError（优先 `RUNTIME_INTERNAL_ERROR`、`retryable=false`），错误详情明确需要人工处理但不泄露敏感信息。
...
recheck admission/cancellation epochs and dispatch fence before adapter and before committing side effects
...
CAS claim pending -> leased with owner_token and lease_until
recheck admission/cancellation epochs and dispatch fence before executor and before any committed result/audit side effect
...
`result_json` 保存完整脱敏 SqlResult。重放只允许修改不参与摘要的 `meta.replayed`。同 invocation 不同 result hash 返回 `SQL_RESULT_CONFLICT`，不得覆盖原结果或再次执行 SQL。SQL 读取若 owner 崩溃且结果未知，只有在执行器/数据源能证明幂等且仍通过 Guard 时才允许 lease 接管；不可证明时内部记录 `unknown`，对外返回 `SQL_EXECUTION_FAILED` 或 `retryable=false` 的既有 RuntimeError，并转人工对账。
...
| admission fencing | admission close/epoch 提升和 active dispatch fence 变化会拒绝旧 invocation 提交副作用 |
| invocation 单执行者 | pending/leased CAS 只允许一个 owner；相同 digest 在请求 deadline 内等待，完成后返回原 Result 且 `meta.replayed=true` |
| invocation 等待超时 | 返回 `RUNTIME_UNAVAILABLE` 503、`retryable=true`；调用方以相同 invocation/idempotency/digest 重试，不创建异步占位或轮询消息 |
| lease 接管 | 过期 lease 仅安全幂等工具可接管；外部副作用 unknown 不自动重放，对外映射既有失败/RuntimeError 并进入人工处理 |
...
SQL Guard、事务、唯一约束和只读账号测试必须在真实 PostgreSQL Testcontainers 中运行；mock 解析器或 SQL 文本断言不足以验收。V2 阶段 4 前必须实际覆盖 pending/leased/terminal/unknown CHECK、owner CAS、过期接管、epoch/fence 竞态和执行计数为 1。
