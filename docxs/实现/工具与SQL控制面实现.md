...
文档定位：本文是 Java Tool 与 SQL 权威执行面的目标实现蓝图，不是完成证明。当前 `foodmate-tool` 和旧 `foodmate-sql-agent` 都只有 marker，`foodmate-infra` 尚无 SQL Guard、只读执行器或 Tool/SQL invocation 持久化。V1 表也未完整持久化工具契约字段，V2 SQL 尚未创建。本文不创建代码或迁移，不新增 ToolProposal、ToolResult、SqlProposal、SqlResult 字段、状态、digest 或错误码。
...
create idempotency placeholder before adapter execution
...
persist status/result_status/result_hash/full ToolResult audit
...
任何 Adapter 执行前必须成功建立 V2 幂等占位。重放从 `output_json/result_status/result_hash` 返回完整 ToolResult，只允许把响应 `meta.replayed` 改为 true；它不参与 result digest。不得重做业务写入。
...
create SQL audit idempotency placeholder
...
redact and cap rows
persist result_status/result_hash/result_json and audit
...
`result_json` 保存完整脱敏 SqlResult。重放只允许修改不参与摘要的 `meta.replayed`。同 invocation 不同 result hash 返回 `SQL_RESULT_CONFLICT`，不得覆盖原结果或再次执行 SQL。
