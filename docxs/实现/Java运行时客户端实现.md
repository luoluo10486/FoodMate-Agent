...
`foodmate-gateway-client` 只负责 Java 到 Python Runtime 的出站通信，不迁移 `AgentRun` 状态，不直接访问数据库，也不承载前端 SSE。目标根包收敛为 `com.foodmate.agentclient`；现有 `com.foodmate.gateway` marker 仅表示当前骨架事实，实施时可保留为模块标记，但不得在两个根包各实现一套客户端。
...
```text
load persisted dispatch
assert now < deadline_at
recompute request_hash from the exact RunCommand digest field set
...
重新派发不是网络重试：只有 Application 做出重新派发决策后，才创建新 `dispatch_id` 且 `attempt=previous+1`。客户端不得自动执行该动作。
