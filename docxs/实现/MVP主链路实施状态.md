...
| 能力 | 位置 | 状态 |
|---|---|---|
| 注册、登录、PBKDF2 密码哈希、Access/Refresh Token | `foodmate-application/.../UserAccountService.java` | 已实现；Refresh Token 仅保存哈希 |
...
| 前端真实接入 | `frontend/src/services` | `VITE_AGENT_MODE=real` 时使用登录、Bearer 请求、Cookie SSE 与取消；默认仍保留 mock |
...
前端真实模式需要设置 `VITE_AGENT_MODE=real`；其余时候默认使用 mock，以支持无后端的原型开发。生产或跨服务环境必须同时配置 Java 的 `RUNTIME_AUTH_TOKEN` 和 Python 的 `FOODMATE_RUNTIME_TOKEN`，并保持 `RUNTIME_CONTRACT_VERSION` 与 `FOODMATE_CONTRACT_VERSION` 一致。
...
- 当前内部认证为共享 Runtime Token 加契约版本头，不是目标文档要求的双向短期 Service JWT/kid。
...
2. 将共享 token 替换为 Service JWT，完成 V1 完整 envelope 与契约测试。
