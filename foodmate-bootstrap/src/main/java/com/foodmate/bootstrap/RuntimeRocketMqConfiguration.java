...
public class RuntimeRocketMqConfiguration {
    ...
    @Bean(initMethod = "start", destroyMethod = "close")
    RocketMqConsumerContainer retentionResultConsumer(
            RocketMqSettings settings,
            DataRetentionResultMessageProcessor processor,
            @Value("${foodmate.retention.rocketmq.result-topic:foodmate-knowledge-purge-result-v1}")
                    String topic,
            @Value(
                            "${foodmate.retention.rocketmq.java-result-consumer-group:foodmate-java-knowledge-purge-result-v1}")
                    String group) {
        return RocketMqConsumerContainer.concurrent(
                settings.nameServer(), group, topic, settings.consumerMaxRetries(), processor);
    }
