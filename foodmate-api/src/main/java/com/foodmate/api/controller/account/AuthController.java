...
import com.foodmate.api.request.account.LoginRequest;
import com.foodmate.api.request.account.PasswordResetConfirmRequest;
import com.foodmate.api.request.account.PasswordResetRequest;
import com.foodmate.api.request.account.RegisterRequest;
import com.foodmate.api.response.account.AuthResponse;
...
public class AuthController {
    ...
    private static String htmlBody(String token, String url) {
        return "<div style='margin:0;background:#f4f7fb;padding:32px 16px;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Microsoft YaHei,sans-serif;color:#172033'>"
                ...
                + "<div style='margin-top:26px;padding:16px;background:#f6f8fb;border-radius:8px;color:#526076;font-size:13px;line-height:1.8'>按钮打不开？没关系，请复制下面的一次性令牌：<br><span style='font-family:Consolas,monospace;color:#172033;word-break:break-all'>"
                + token
                + "</span></div>"
    ...
    private ResponseEntity<ApiResponse<AuthResponse>> response(
            UserAccountService.AuthResult result) {
        AuthResponse body =
                new AuthResponse(
    ...
    private static UserAccountService.SessionMetadata metadata(
            jakarta.servlet.http.HttpServletRequest request) {
        return new UserAccountService.SessionMetadata(
                request.getHeader("User-Agent"), request.getRemoteAddr());
    }

    private ResponseCookie cookie(
            String name, String value, java.time.Instant expiresAt, boolean httpOnly) {
        return ResponseCookie.from(name, value)
                .httpOnly(httpOnly)
                .secure(secureCookie)
                .sameSite("Lax")
                .path("/")
                .maxAge(java.time.Duration.between(java.time.Instant.now(), expiresAt))
                .build();
    }
