...
class CsrfProtectionMatrixTest {
    ...
    void sameOriginMutationWithValidCsrfContinues() throws Exception {
        ...
        new CsrfProtectionFilter(provider(service))
    ...
    void missingOrInvalidCsrfIsRejected() throws Exception {
        ...
        doThrow(
                ...
                .when(service)
                .requireCsrf("session-token", null);
        FilterChain chain = mock(FilterChain.class);
    ...
    void safeMethodsBypassCsrfChecks() throws Exception {
        ...
        FilterChain chain = mock(FilterChain.class);
        MockHttpServletRequest request =
                request("GET", "https://app.example.test", "https://evil.example.test", null);
        new CsrfProtectionFilter(provider(service))
                .doFilter(request, new MockHttpServletResponse(), chain);
        verify(chain).doFilter(eq(request), org.mockito.ArgumentMatchers.any());
    ...
    private static MockHttpServletRequest request(
            String method, String server, String origin, String csrf) {
        java.net.URI uri = java.net.URI.create(server);
        MockHttpServletRequest request =
                new MockHttpServletRequest(
                        "POST".equals(method) ? "POST" : method, "/api/sessions");
        request.setScheme(uri.getScheme());
        request.setServerName(uri.getHost());
        request.setServerPort(
                uri.getPort() < 0 ? ("https".equals(uri.getScheme()) ? 443 : 80) : uri.getPort());
    ...
    private static ObjectProvider<UserAccountService> provider(UserAccountService service) {
        return new ObjectProvider<>() {
            public UserAccountService getObject(Object... args) {
                return service;
            }

            public UserAccountService getIfAvailable() {
                return service;
            }

            public UserAccountService getIfUnique() {
                return service;
            }

            public Stream<UserAccountService> orderedStream() {
                return Stream.of(service);
            }

            public Stream<UserAccountService> stream() {
                return Stream.of(service);
            }
        };
    }
