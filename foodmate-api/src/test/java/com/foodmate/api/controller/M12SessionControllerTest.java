    ...
    void supportsPagingSoftDeleteRestoreAndUserOnlyMessages() throws Exception {
        mockMvc.perform(
                post("/api/auth/register")
                        ...
                        .content(
                                "{\"username\":\"m12\",\"email\":\"m12@example.com\",\"password\":\"password123\"}"));
        var login =
                mockMvc.perform(
                                post("/api/auth/login")
                                        .contentType(MediaType.APPLICATION_JSON)
                                        .content(
                                                "{\"username_or_email\":\"m12\",\"password\":\"password123\"}"))
                        .andReturn()
                        .getResponse();
        ...
        mockMvc.perform(
                        delete("/api/sessions/" + id)
                                ...
                                .header("X-CSRF-Token", csrf.getValue()))
                .andExpect(status().isOk());
        mockMvc.perform(get("/api/sessions/deleted").cookie(session))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.items[0].status", is("deleted")));
        mockMvc.perform(
