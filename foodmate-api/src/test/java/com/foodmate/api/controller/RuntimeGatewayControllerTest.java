...
class RuntimeGatewayControllerTest {
    ...
    static void properties(DynamicPropertyRegistry registry) {
        ...
        registry.add(
                ...
                () -> encoded(JAVA_KEYS.getPublic()));
        registry.add(
                "foodmate.runtime.service-jwt.python-public-key",
                () -> encoded(PYTHON_KEYS.getPublic()));
    }

    @Test
    void dispatchRetryIsDuplicateAndChangedBodyConflicts() throws Exception {
        mockMvc.perform(
                        post("/internal/runtime/runs:dispatch")
                                .header("Authorization", "Bearer " + javaToken("runtime:dispatch"))
    ...
    @Test
    void eventChainReachesSucceededAndRejectsRollback() throws Exception {
        mockMvc.perform(
                post("/internal/runtime/runs:dispatch")
                        .header("Authorization", "Bearer " + javaToken("runtime:dispatch"))
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    private void postEvent(String id, int seq, String state, String payload, boolean conflict)
            throws Exception {
        String json =
                "{\"event_id\":\""
                        + id
                        + "\",\"run_id\":\"r-http\",\"event_seq\":"
                        + seq
                        + ",\"state\":\""
                        + state
                        + "\",\"payload\":"
        ...
        var result =
                mockMvc.perform(
                        post("/internal/runtime/runs:events")
                                .header("Authorization", "Bearer " + pythonToken("runtime:event"))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(json));
        if (conflict)
    ...
    private static String javaToken(String scope) {
        return ServiceJwt.sign(
                encoded(JAVA_KEYS.getPrivate()),
    ...
    private static String pythonToken(String scope) {
        return ServiceJwt.sign(
                encoded(PYTHON_KEYS.getPrivate()),
