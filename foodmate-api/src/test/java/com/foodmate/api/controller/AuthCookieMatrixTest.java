...
class AuthCookieMatrixTest {
    ...
    void productionCookiesHaveSecureHttpOnlyAndSameSiteAttributes() throws Exception {
        ...
        String session =
                cookies.stream()
                        .filter(value -> value.startsWith("foodmate_session="))
                        .findFirst()
                        .orElseThrow();
        String csrf =
                cookies.stream()
    ...
    void localCookiesMayOmitSecureButKeepOtherAttributes() throws Exception {
        List<String> cookies =
                MockMvcBuilders.standaloneSetup(new AuthController(mockService(), false))
                        .build()
                        .perform(
                                org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                        .post("/api/auth/login")
    ...
    void logoutExpiresAllAuthenticationCookies() throws Exception {
        List<String> cookies =
                MockMvcBuilders.standaloneSetup(new AuthController(mockService(), true))
                        .build()
                        .perform(
                                org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                        ...
                                        .cookie(
                                                new jakarta.servlet.http.Cookie(
                                                        "foodmate_session", "session-token")))
                        .andReturn()
                        .getResponse()
                        .getHeaders("Set-Cookie");
        assertThat(cookies)
