...
public class MemoryCandidateService {
    ...
    public List<MemoryView> list(long userId) {
        return store.findVisible(userId, 100).stream().map(MemoryCandidateService::view).toList();
    }
    ...
    private MemoryView get(long userId, long memoryId) {
        MemoryStore.MemorySnapshot memory = store.findOwned(userId, memoryId);
    ...
    private static MemoryView view(MemoryStore.MemorySnapshot value) {
