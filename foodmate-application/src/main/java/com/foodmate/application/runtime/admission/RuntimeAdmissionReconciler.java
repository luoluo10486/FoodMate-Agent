...
public class RuntimeAdmissionReconciler {
    ...
    public void reconcile() {
        ...
        List<AdmissionReconciliationRepository.RunRef> queued =
                store.findQueueExpired(queueTimeoutSeconds, 20);
        queued.forEach(run -> fail(run, "RUNTIME_QUEUE_TIMEOUT", "Agent 排队超时"));

        List<AdmissionReconciliationRepository.RunRef> expired = store.findExecutionExpired(20);
        ...
        try {
            List<String> promoted =
                    store.findQueued(20).stream()
                            ...
                            .map(AdmissionReconciliationRepository.RunRef::runId)
                            .toList();
    ...
    protected void fail(AdmissionReconciliationRepository.RunRef run, String code, String message) {
