...
public class SessionSummaryServiceImpl implements SessionSummaryService {
    ...
    private final ConversationSummaryRepository store;
    private final IdGenerator ids;
    private final ObjectMapper mapper = new ObjectMapper();

    public SessionSummaryServiceImpl(ConversationSummaryRepository store, IdGenerator ids) {
        this.store = store;
        this.ids = ids;
    }
    ...
    public void maybeRefresh(long userId, long sessionId) {
        ...
        List<ConversationSummaryRepository.MessageSnapshot> messages =
        ...
        if (current == null) {
            store.insertSummary(
                    new ConversationSummaryRepository.NewSummary(
                            ids.nextId(),
                            sessionId,
                            summary,
                            structured,
            ...
            int changed =
                    store.updateSummary(
                            new ConversationSummaryRepository.UpdatedSummary(
                                    current.id(),
                                    current.version(),
                                    summary,
                                    structured,
    ...
    private boolean isGoal(ConversationSummaryRepository.MessageSnapshot item) {
        return MessageRole.USER.code().equals(item.role())
                && item.content().matches(".*(想|希望|计划|目标|安排|帮我).*");
    }

    private boolean isConstraint(ConversationSummaryRepository.MessageSnapshot item) {
