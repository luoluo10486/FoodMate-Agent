...
- `persistence`：数据库访问层，封装 MyBatis-Plus Mapper / Service
...
FoodMate 里建议统一走 `ModelService` 的地方包括：
...
注意：本节来自模型配置示例整理，最终业务表字段命名以 `FoodMate-数据库设计与接口工具清单.md` 为准；业务表主键统一使用 `{业务对象}_id`，不要使用泛化 `id`。
