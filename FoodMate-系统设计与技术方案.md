...
FoodMate 建议采用更通用的 AI 应用分层模型，这样后续无论增加新 Agent、新工具还是新模型，都能保持工程边界清晰。

```mermaid
flowchart TD
  A[产品应用层\nFoodMate AI 客服 / 知识库 / 办公助手 / 行业 Agent]
...
#### 4.3.6 FoodMate 的基础设施映射

| 基础设施模块 | FoodMate 对应实现 |
|---|---|
...
| 能力 | 核心作用 | 适合场景 | 风险级别 | FoodMate 优先级 |
|---|---|---|---|---|
...
##### 4.3.7.8 FoodMate 的必要工具清单

如果从“能跑起来、能交付、能扩展”的角度看，FoodMate 的工具不需要一开始就很多。  
真正必要的，是下面这组**最小可用工具集**：
...
对于“知识库找不到，但用户还想知道更详细的公开信息”这类问题，推荐不要直接把 Browser Use 放在第一位，而是按下面的顺序处理：

1. **先查内部知识库**
   - 优先命中本地知识、业务文档、结构化知识库

2. **再查外部结构化来源**
   - 优先使用百科 API、公开食材库、营养数据库、权威公开数据源
   - 如果能用 API，不要先上浏览器
...
- 浏览器操作保留
- 但不要作为默认工具
- 先让知识库、外部检索、结构化数据源解决问题
- Browser Use 只在没有更好办法时启用
...
FoodMate 的 Agent 框架不建议一开始就“全家桶式接入”。  
...
##### 4.3.9.7 FoodMate 落地建议

FoodMate 里建议把下面这些都放进 `ModelService`：
...
FoodMate 可以在这个基础上再补一张 `model_route_rules`，这样就从“可用”升级成“可治理”。
...
RAG 不要直接拿用户原始问题去检索，建议在检索前加入“查询理解层”。这一层不只是改写 query，还要把自然语言问题整理成更适合召回、过滤和审计的结构化输入。
...
- 适合 Milvus sparse BM25 的关键词 query
- 适合 Milvus dense vector 的语义 query
- 适合过滤条件抽取的结构化约束
...
- 能继承上下文的，先继承上下文
- 无法确定时，要么追问，要么保守处理
- 高风险问题在查询理解后仍要保留原始 query 用于审计

#### 查询理解的核心步骤

在 FoodMate 里，查询理解层至少包含三步：
...
- 查询理解层负责“把问题变成可检索表达”
- Milvus sparse BM25 负责 MVP 阶段的关键词召回
- Milvus dense vector 负责语义召回
- rerank 负责“最终排序”
- Elasticsearch 只作为后续搜索分析增强的可选补充，不进入第一版主检索链路

### 10.7 Agent 行为范式

FoodMate 建议采用 **Plan-Act-Observe-Reflect** 的行为范式，并结合有限状态机控制执行边界。
...
FoodMate 的知识库不只是“把文档丢进向量库”，而是一个完整的构建流水线。
...
除了知识库检索，FoodMate 还需要一个受控的数据查询层，用于访问业务数据库、报表库、统计库和用户行为库。
...
2. Planner 判断需要营养口径和克重换算
3. 调用 `knowledge_search` 获取营养数据来源，必要时再调用 `calculator`
4. 返回估算营养值
5. 输出结果并注明数据来源和口径
...
如果你要继续往下做，建议按下面的顺序阅读并推进：

1. **FoodMate-产品需求文档.md**
2. **FoodMate-系统设计与技术方案.md**
3. **FoodMate-智能体行为与工具协议.md**
4. **FoodMate-接口与数据规范.md**
5. **FoodMate-Java工程骨架与模块设计.md**
6. **FoodMate-数据库设计与接口工具清单.md**
7. **FoodMate-实现附录.md**

这样就可以从“需求与架构”逐步进入“接口、工程骨架、数据库与实现细节”。
